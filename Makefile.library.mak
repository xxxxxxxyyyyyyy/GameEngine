...
ifeq ($(OS),Windows_NT)
	...
	EXTENSION := .dll
	COMPILER_FLAGS := -Wall -Wextra -Werror -Wvla -Wgnu-folding-constant -Wno-missing-braces -fdeclspec -Wstrict-prototypes -Wno-unused-parameter -Wno-missing-field-initializers -Wno-unused-but-set-variable -Wno-unused-function
...
ifeq ($(BUILD_PLATFORM),windows)
	...
	DVERSION := $(shell $(DIR)\$(BUILD_DIR)\versiongen.exe "$(VERFILE_TEXT)")
else
	VERFILE := $(ASSEMBLY)/src/$(ASSEMBLY)_version.h
	VERFILE_TEXT := $(ASSEMBLY)/version.txt
	DVERSION := $(shell $(BUILD_DIR)/versiongen "$(VERFILE_TEXT)")
endif
