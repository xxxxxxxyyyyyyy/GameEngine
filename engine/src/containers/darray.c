...
void* _darray_create(u64 length, u64 stride, frame_allocator_int* allocator) {
    ...
    if (length == 0) {
        DFATAL("_darray_create called with length of 0");
    }
...
void* _darray_resize(void* array) {
    ...
    if (header->capacity == 0) {
        DFATAL("_darray_resize called on an array with 0 capacity. This should not be possible.");
        return 0;
    }
...
void _darray_pop(void* array, void* dest) {
    ...
    if (length < 1) {
        DERROR("darray_pop called on an empty darray. Nothing to be done.");
        return;
    }
    u64 addr = (u64)array;
