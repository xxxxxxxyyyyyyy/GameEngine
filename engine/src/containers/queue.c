...
b8 queue_create(queue* out_queue, u32 element_size) {
    if (!out_queue) {
        DERROR("queue_create requires a pointer to a valid queue.");
        return false;
    }
...
b8 queue_push(queue* s, void* element_data) {
    if (!s) {
        DERROR("queue_push requires a pointer to a valid queue.");
        return false;
    }
...
b8 queue_peek(const queue* s, void* out_element_data) {
    if (!s || !out_element_data) {
        DERROR("queue_peek requires a pointer to a valid queue and to hold element data output.");
        return false;
    }

    if (s->element_count < 1) {
        DWARN("Cannot peek from an empty queue.");
        return false;
    }
...
b8 queue_pop(queue* s, void* out_element_data) {
    if (!s || !out_element_data) {
        DERROR("queue_pop requires a pointer to a valid queue and to hold element data output.");
        return false;
    }

    if (s->element_count < 1) {
        DWARN("Cannot pop from an empty queue.");
        return false;
    }
