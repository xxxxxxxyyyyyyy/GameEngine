...
b8 ring_queue_create(u32 stride, u32 capacity, void* memory, ring_queue* out_queue) {
    if (!out_queue) {
        DERROR("ring_queue_create requires a valid pointer to hold the queue.");
        return false;
    }
...
b8 ring_queue_enqueue(ring_queue* queue, void* value) {
    if (queue && value) {
        if (queue->length == queue->capacity) {
            DERROR("ring_queue_enqueue - Attempted to enqueue value in full ring queue: %p", queue);
            return false;
        }
    ...
    DERROR("ring_queue_enqueue requires valid pointers to queue and value.");
    return false;
}
...
b8 ring_queue_dequeue(ring_queue* queue, void* out_value) {
    if (queue && out_value) {
        if (queue->length == 0) {
            DERROR("ring_queue_dequeue - Attempted to dequeue value in empty ring queue: %p", queue);
            return false;
        }
    ...
    DERROR("ring_queue_dequeue requires valid pointers to queue and out_value.");
    return false;
}
...
b8 ring_queue_peek(const ring_queue* queue, void* out_value) {
    if (queue && out_value) {
        if (queue->length == 0) {
            DERROR("ring_queue_peek - Attempted to peek value in empty ring queue: %p", queue);
            return false;
        }
    ...
    DERROR("ring_queue_peek requires valid pointers to queue and out_value.");
    return false;
}