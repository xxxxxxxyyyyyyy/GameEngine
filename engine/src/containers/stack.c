...
b8 stack_create(stack* out_stack, u32 element_size) {
    if (!out_stack) {
        DERROR("stack_create requires a pointer to a valid stack.");
        return false;
    }
...
b8 stack_push(stack* s, void* element_data) {
    if (!s) {
        DERROR("stack_push requires a pointer to a valid stack.");
        return false;
    }
...
b8 stack_peek(const stack* s, void* out_element_data) {
    if (!s || !out_element_data) {
        DERROR("stack_peek requires a pointer to a valid stack and to hold element data output.");
        return false;
    }

    if (s->element_count < 1) {
        DWARN("Cannot peek from an empty stack.");
        return false;
    }
...
b8 stack_pop(stack* s, void* out_element_data) {
    if (!s || !out_element_data) {
        DERROR("stack_pop requires a pointer to a valid stack and to hold element data output.");
        return false;
    }

    if (s->element_count < 1) {
        DWARN("Cannot pop from an empty stack.");
        return false;
    }
