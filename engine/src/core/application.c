...
// UI view
    ...
    if (!render_view_system_create(&pick_view_config)) {
        DFATAL("Failed to create pick view. Aborting application.");
        return false;
    }
    ...
    if (!ui_text_create(UI_TEXT_TYPE_BITMAP, "Ubuntu Mono 21px", 21, "Some text 123,\n\t李明安", &app_state->test_text)) {
        DERROR("Failed to load basic ui bitmap text.");
        return false;
    }
    ui_text_set_position(&app_state->test_text, vec3_create(50, 100, 0));
    if(!ui_text_create(UI_TEXT_TYPE_SYSTEM, "Noto Sans CJK JP", 31, "2222222222222, \n\tyo!\n\n\t何志远", &app_state->test_sys_text)) {
    ...
    if(!ui_text_create(UI_TEXT_TYPE_SYSTEM, "Noto Sans CJK JP", 31, "66666666666, \n\tyo!\n\n\t111111", &app_state->test_sys_text)) {
        DERROR("Failed to load basic ui system text.");
        return false;
    }
    ...
    const f32 width = 96.0f;
    const f32 height = 48.0f;
    vertex_2d uiverts [4];
