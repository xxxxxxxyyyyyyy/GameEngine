...
b8 console_command_register(const char* command, u8 arg_count, PFN_console_command func) {
    ...
    for (u32 i = 0; i < command_count; ++i) {
        if (strings_equali(state_ptr->registered_commands[i].name, command)) {
            DERROR("Command already registered: %s", command);
            return false;
        }
    }
...
static void console_object_print(u8 indent, console_object* obj) {
    ...
    switch (obj->type) {
        case CONSOLE_OBJECT_TYPE_INT32:
            DINFO("%s%i", indent_buffer, *((i32*)obj->block));
            break;
        case CONSOLE_OBJECT_TYPE_UINT32:
            DINFO("%s%u", indent_buffer, *((u32*)obj->block));
            break;
        case CONSOLE_OBJECT_TYPE_F32:
            DINFO("%s%f", indent_buffer, *((f32*)obj->block));
            break;
        case CONSOLE_OBJECT_TYPE_BOOL: {
            b8 val = *((b8*)obj->block);
            DINFO("%s%s", indent_buffer, val ? "true" : "false");
        } break;
...
static b8 console_operator_type_from_token(const console_token token, const char* source, console_operator_type* out_type) {
    if (!source || token.type != CONSOLE_TOKEN_TYPE_OPERATOR) {
        DERROR("console_operator_type_from_token requires a valid pointer to source and an operator type token");
        return false;
    }

    // NOTE: Ensure token length here so it doesn't have to be verified anywhere below.
    if (token.length < 1 || token.length > 2) {
        DERROR("operators are always either 1 or 2 characters.");
        return false;
    }
    ...
    *out_type = CONSOLE_OPERATOR_TYPE_UNKNOWN;
    ...
    if (!result) {
        DERROR("Unexpected character '%c' at position %u.", source[token.start_index + error_offset], token.start_index + error_offset);
    }
    return result;
}
...
static b8 tokenize_source(const char* source, stack* out_tokens) {
    if (!source || !out_tokens) {
        DERROR("tokenize_source requires valid pointers to source and out_tokens.");
        return false;
    }
    ...
    for (u32 i = 0; i < char_length; ++i) {
        ...
        if (!bytes_to_codepoint(source, i, &codepoint, &advance)) {
            ...
            DERROR("Error processing expression - unable to decode codepoint at position: %u", i);
            return false;
        }
            ...
            case '%': {
                ...
                if (current_token.type != CONSOLE_TOKEN_TYPE_OPERATOR) {
                    ...
                    if (current_token.length >= 2) {
                        DERROR("Invalid character at position %u.", i);
                        return false;
                    }
                        ...
                        case '=': {
                            ...
                            switch (prev) {
                                ...
                                case '%':
                                    ...
                                    break;
                                default:
                                    DERROR("Unexpected character at position %u", i);
                                    return false;
                            }
                        }
                        case '+':
    ...
    if (token->type == CONSOLE_TOKEN_TYPE_NUMERIC_LITERAL) {
        ...
        if (string_index_of(value, '.')) {
            if (!string_to_f32(value, &out_result->f)) {
                DERROR("Failed to parse token as float at position %i.", token->start_index);
                result = false;
            }
            *out_type = CONSOLE_OBJECT_TYPE_F32;
            ...
            if (!string_to_i32(value, &out_result->i)) {
                DERROR("Failed to parse token as integer at position %i.", token->start_index);
                result = false;
            }
        ...
        string_sub(name, source, token->start_index, token->length);
        console_object* obj = console_object_get(0, name);
        if (!obj) {
            DERROR("Identifier '%s' is undefined.", name);
            result = false;
        } else {
            switch (obj->type) {
                case CONSOLE_OBJECT_TYPE_INT32:
                ...
                default:
                    // Wrong type of token.
                    DERROR("Expected numeric token (i32, u32 or f32) at position %i", token->start_index);
                    result = false;
                    break;
            }
        }
...
static b8 evaluate_numerical_expression(const char* source, const console_token* a, const console_token* operator, const console_token* b, console_object_type* out_type, console_numeric* out_result) {
    ...
    if (!console_operator_type_from_token(*operator, source, &operator_type)) {
        DERROR("Failed to evaluate numerical expression due to invalid operator token.");
        return false;
    }

    // Obtain token values.
    if (numeric_token_parse(source, a, &type_a, &numeric_a)) {
        DERROR("Failed to evaluate numerical expression. See logs for details.");
        return false;
    }
    ...
    if (is_unary) {
        if (type_a == CONSOLE_OBJECT_TYPE_F32) {
            if (operator_type == CONSOLE_OPERATOR_TYPE_INCREMENT) {
                DWARN("Incrementing on float at position %i. May yield unexpected results.", a->start_index);
                out_result->f = numeric_a.f + 1;
            } else if (operator_type == CONSOLE_OPERATOR_TYPE_DECREMENT) {
                DWARN("Decrementing on float at position %i. May yield unexpected results.", a->start_index);
                out_result->f = numeric_a.f - 1;
            } else {
                DERROR("Unexpected unary operator at position %i", a->start_index);
                return false;
            }
        } else {
            // Treat as int.
        ...
        if (type_a == CONSOLE_OBJECT_TYPE_F32 || type_b == CONSOLE_OBJECT_TYPE_F32) {
            *out_type = CONSOLE_OBJECT_TYPE_F32;
            ...
            if (type_a == CONSOLE_OBJECT_TYPE_F32) {
                ...
                DERROR("Unsupported object type at position %i.", a->start_index);
                return false;
            }
            if (type_b == CONSOLE_OBJECT_TYPE_F32) {
                ...
                DERROR("Unsupported object type at position %i.", b->start_index);
                return false;
            }
...
static b8 console_expression_parse(const char* expression, console_object_type* out_type, void* out_value) {
    ...
    if (!operator_found) {
        ...
        if (space_index != -1) {
            DERROR("Unexpected token at position %i", space_index);
            result = false;
        ...
        if (!identifier_found) {
            DERROR("Identifier is undefined: '%s'.", expression_copy);
            result = false;
            goto console_expression_parse_cleanup;
        }
    }
...
b8 console_command_execute(const char* command) {
    ...
    for (u32 i = 0; i < command_count; ++i) {
        ...
        if (strings_equali(cmd->name, parts[0])) {
            ...
            if (state_ptr->registered_commands[i].arg_count != arg_count) {
                DERROR("The console command '%s' requires %u arguments but %u were provided.", cmd->name, cmd->arg_count, arg_count);
                has_error = true;
            } else {
    ...
    if (!command_found) {
        DERROR("The command '%s' does not exist.", string_trim(parts[0]));
        has_error = true;
    }
...
b8 console_object_register(const char* object_name, void* object, console_object_type type) {
    if (!object || !object_name) {
        DERROR("console_object_register requires a valid pointer to object and object_name");
        return false;
    }
    ...
    for (u32 i = 0; i < object_count; ++i) {
        if (strings_equali(state_ptr->registered_objects[i].name, object_name)) {
            DERROR("Console object already registered: '%s'.", object_name);
            return false;
        }
    }
...
b8 console_object_unregister(const char* object_name) {
    if (!object_name) {
        DERROR("console_object_register requires a valid pointer object_name");
        return false;
    }
...
b8 console_object_add_property(const char* object_name, const char* property_name, void* property, console_object_type type) {
    if (!property || !object_name || !property_name) {
        DERROR("console_object_add_property requires a valid pointer to property, property_name and object_name");
        return false;
    }
            ...
            if (obj->properties) {
                ...
                for (u32 j = 0; j < property_count; ++j) {
                    if (strings_equali(obj->properties[j].name, property_name)) {
                        DERROR("Object '%s' already has a property named '%s'.", object_name, property_name);
                        return false;
                    }
                }
            } else {
    ...
    DERROR("Console object not found: '%s'.", object_name);
    return false;
}
...
b8 console_object_remove_property(const char* object_name, const char* property_name) {
    if (!object_name || !property_name) {
        DERROR("console_object_remove_property requires a valid pointer to property, property_name and object_name");
        return false;
    }
    ...
    for (u32 i = 0; i < object_count; ++i) {
        if (strings_equali(state_ptr->registered_objects[i].name, object_name)) {
            ...
            DERROR("Property '%s' not found on console object '%s'.", object_name, property_name);
            return false;
        }
    }
