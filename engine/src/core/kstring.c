...
u32 string_utf8_length(const char* str) {
    ...
    for (u32 i = 0; i < __UINT32_MAX__; ++i, ++length) {
        ...
        if (c >= 0 && c < 127) {
            ...
            DERROR("kstring string_utf8_length() - Not supporting 5 and 6-byte characters; Invalid UTF-8.");
            return 0;
        }
    }
...
b8 bytes_to_codepoint(const char* bytes, u32 offset, i32* out_codepoint, u8* out_advance) {
    ...
    if (codepoint >= 0 && codepoint < 0x7F) {
        ...
        *out_advance = 0;
        *out_codepoint = 0;
        DERROR("kstring bytes_to_codepoint() - Not supporting 5 and 6-byte characters; Invalid UTF-8.");
        return false;
    }
}
...
void string_sub(char* dest, const char* source, i32 start, i32 length) {
    if (length == 0) {
        return;
    }
...
b8 string_to_mat4(const char* str, matrix4* out_mat) {
    if (!str || !out_mat) {
        return false;
    }

    kzero_memory(out_mat, sizeof(matrix4));
...
void string_filename_no_extension_from_path(char* dest, const char* path) {
    ...
    string_sub(dest, path, start, end - start);
}
...
b8 string_parse_array_length(const char* str, u32* out_length) {
    ...
    string_sub(num_string, str, open_index + 1, close_index - open_index);
...
void kstring_create(kstring* out_string) {
    if (!out_string) {
        DERROR("kstring_create requires a valid pointer to a string.");
        return;
    }
...
void kstring_from_cstring(const char* source, kstring* out_string) {
    if (!out_string) {
        DERROR("kstring_from_cstring requires a valid pointer to a string.");
        return;
    }
