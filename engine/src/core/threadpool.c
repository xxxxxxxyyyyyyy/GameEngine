...
b8 threadpool_create(u32 thread_count, threadpool* out_pool) {
    if (!thread_count || !out_pool) {
        DERROR("threadpool_create requires at least 1 thread and a valid pointer to hold the created pool.");
        return false;
    }
    ...
    for (u32 i = 0; i < thread_count; ++i) {
        if (!worker_thread_create(&out_pool->threads[i])) {
            DERROR("Error creating worker thread. threadpool_create failed.");
            return false;
        }
    }
...
b8 threadpool_wait(threadpool* pool) {
    if (!pool) {
        DERROR("threadpool_wait requires a valid pointer to a thread pool.");
        return false;
    }
    ...
    for (u32 i = 0; i < pool->thread_count; ++i) {
        if (!worker_thread_wait(&pool->threads[i])) {
            DERROR("Failed to wait for worker thread in thread pool. See logs for details.");
            success = false;
        }
        DTRACE("Worker thread wait complete.");
    }

    if (!success) {
        DERROR("There was an error waiting for the threadpool. See logs for details.");
    }

    DTRACE("Done waiting on all threads");
