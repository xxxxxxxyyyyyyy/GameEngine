...
static u32 worker_thread_loop(void* params) {
    ...
    while (true) {
        ...
        if (!queue_pop(&thread->work_queue, &w)) {
            DERROR("Failed to pop work from work queue.");
            kmutex_unlock(&thread->queue_mutex);
            return 0;
        }
        kmutex_unlock(&thread->queue_mutex);
    ...
    DTRACE("Worker thread work complete.");
...
b8 worker_thread_create(worker_thread* out_thread) {
    ...
    if (!queue_create(&out_thread->work_queue, sizeof(work))) {
        DERROR("Failed to create internal work queue for worker thread.");
        return false;
    }
    if (!kmutex_create(&out_thread->queue_mutex)) {
        DERROR("Failed to create internal work queue mutex for worker thread.");
        return false;
    }
...
b8 worker_thread_add(worker_thread* thread, pfn_thread_start work_fn, void* work_params) {
    if (!thread || !work_fn) {
        DERROR("worker_thread_add requires valid pointers to a worker_thread and a work function pointer.");
        return false;
    }
    ...
    if (!queue_push(&thread->work_queue, &w)) {
        DERROR("Failed to push work into queue.");
        kmutex_unlock(&thread->queue_mutex);
        return false;
    }
    kmutex_unlock(&thread->queue_mutex);
...
b8 worker_thread_start(worker_thread* thread) {
    ...
    if (!kthread_create(worker_thread_loop, thread, false, &thread->thread)) {
        DERROR("Worker thread internal thread creation failed.");
        return false;
    }
