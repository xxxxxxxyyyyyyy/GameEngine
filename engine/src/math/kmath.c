...
f32 kattenuation_min_max(f32 min, f32 max, f32 x) {
    ...
    f32 att = CLAMP((half_range - distance) / half_range, 0, 1);
    return att;
}
...
frustum frustum_from_view_projection(matrix4 view_projection) {
    frustum f;

    // Get the inverse of the view_projection matrix.
    matrix4 inv = mat4_inverse(view_projection);
    f32 *md = inv.data;
    ...
    vec4 matrix3 = {md[12], md[13], md[14], md[15]};

    // Calculate the projection planes and normalize them, including distances.
    vec4 sides[6];
    sides[FRUSTUM_SIDE_LEFT] = vec4_normalized(vec4_add(matrix3, mat0));
    sides[FRUSTUM_SIDE_RIGHT] = vec4_normalized(vec4_sub(matrix3, mat0));
    sides[FRUSTUM_SIDE_TOP] = vec4_normalized(vec4_sub(matrix3, mat1));
    sides[FRUSTUM_SIDE_BOTTOM] = vec4_normalized(vec4_add(matrix3, mat1));
    sides[FRUSTUM_SIDE_NEAR] = vec4_normalized(vec4_add(matrix3, mat2));
    sides[FRUSTUM_SIDE_FAR] = vec4_normalized(vec4_sub(matrix3, mat2));
...
void frustum_corner_points_world_space(matrix4 projection_view, vec4 *corners) {
    matrix4 inverse_view_proj = mat4_inverse(projection_view);
