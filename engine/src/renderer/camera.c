...
matrix4 camera_view_get(camera* c) {
    if (c) {
        if (c->is_dirty) {
            matrix4 rotation = mat4_euler_xyz(c->euler_rotation.x, c->euler_rotation.y, c->euler_rotation.z);
            matrix4 translation = mat4_translation(c->position);
...
vec3 camera_forward(camera* c) {
    if (c) {
        matrix4 view = camera_view_get(c);
...
void camera_pitch(camera* c, f32 amount) {
    if (c) {
        ...
        c->euler_rotation.x = CLAMP(c->euler_rotation.x, -limit, limit);
