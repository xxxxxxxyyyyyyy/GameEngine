...
b8 forward_rendergraph_frame_prepare(forward_rendergraph* graph, struct frame_data* p_frame_data, struct camera* current_camera, struct viewport* current_viewport, struct simple_scene* scene, u32 render_mode) {
    ...
    if (scene->state == SIMPLE_SCENE_STATE_LOADED) {
        ...
        matrix4 shadow_camera_lookats[MAX_CASCADE_COUNT];
        matrix4 shadow_camera_projections[MAX_CASCADE_COUNT];
        ...
        if (dir_light) {
            ...
            matrix4 shadow_dist_projection = mat4_perspective(
                current_viewport->fov,
                current_viewport->rect.width / current_viewport->rect.height,
                near,
                far);
            matrix4 cam_view_proj = mat4_transposed(mat4_mul(camera_view_get(current_camera), shadow_dist_projection));
        ...
        // Scene pass.
        {
            ...
            matrix4 camera_projection = current_viewport->projection;
