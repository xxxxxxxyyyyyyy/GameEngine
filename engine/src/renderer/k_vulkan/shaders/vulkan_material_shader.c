...
#include "renderer/k_vulkan/vulkan_shader_utils.h"
#include "renderer/k_vulkan/vulkan_pipeline.h"
#include "renderer/k_vulkan/vulkan_buffer.h"
...
#define ATTRIBUTE_COUNT 2
    ...
    if (!vulkan_buffer_create(
        ...
        ERROR("Material instance buffer creation failed for shader.");
        return false;
    }
...
void vulkan_material_shader_update_object(vulkan_context* context, struct vulkan_material_shader* shader, geometry_render_data data) {
	...
	// 128 bytes fot the pushconstant per
    vkCmdPushConstants(command_buffer, shader->pipeline.pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(matrix4), &data.model);

    // Obtain material data.
    vulkan_material_shader_instance_state* instance_state = &shader->material_instance_states[data.material->internal_id];
    ...
    for (u32 sampler_index = 0; sampler_index < sampler_count; ++sampler_index) {
        ...
        switch (use) {
            case TEXTURE_USE_MAP_DIFFUSE:
                ...
                break;
            default:
                FATAL("Unable to bind sampler to unknown use.");
                return;
        }
    ...
    // we need to update the desriptor before we bind it.
    // because not all gpu can support bind first.
...
b8 vulkan_material_shader_acquire_resources(vulkan_context* context, struct vulkan_material_shader* shader, material* material) {
    ...
    vulkan_material_shader_instance_state* instance_state = &shader->material_instance_states[material->internal_id];
    for (u32 i = 0; i < VULKAN_MATERIAL_SHADER_DESCRIPTOR_COUNT; ++i) {
        for (u32 j = 0; j < 3; ++j) {
