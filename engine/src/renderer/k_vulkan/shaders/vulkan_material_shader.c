...
void vulkan_material_shader_set_model(vulkan_context* context, struct vulkan_material_shader* shader, matrix4 modelMatrix) {
    if (context && shader) {
        u32 image_index = context->image_index;
        VkCommandBuffer command_buffer = context->graphics_command_buffers[image_index].handle;

        vkCmdPushConstants(command_buffer, shader->pipeline.pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(matrix4), &modelMatrix);
    }
}
...
void vulkan_material_shader_apply_material(vulkan_context* context, struct vulkan_material_shader* shader, material* material) {
    if (context && shader) {
        ...
        vulkan_material_shader_instance_state* object_state = &shader->material_instance_states[material->internal_id];
