...
b8 vulkan_ui_shader_create(vulkan_context* context, vulkan_ui_shader* out_shader) {
    ...
    for (u32 i = 0; i < UI_SHADER_STAGE_COUNT; ++i) {
        if (!create_shader_module(context, BUILTIN_SHADER_NAME_UI, stage_type_strs[i], stage_types[i], i, out_shader->stages)) {
            ERROR("Unable to create %s shader module for '%s'.", stage_type_strs[i], BUILTIN_SHADER_NAME_UI);
            return false;
        }
    }
...
#define ATTRIBUTE_COUNT 2
    ...
    if (!vulkan_buffer_create(
        ...
        ERROR("UI instance buffer creation failed for shader.");
        return false;
    }
...
void vulkan_ui_shader_set_model(vulkan_context* context, struct vulkan_ui_shader* shader, matrix4 model) {
    if (context && shader) {
        u32 image_index = context->image_index;
        VkCommandBuffer command_buffer = context->graphics_command_buffers[image_index].handle;

        vkCmdPushConstants(command_buffer, shader->pipeline.pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(matrix4), &model);
    }
}
