...
b8 renderer_system_initialize(u64* memory_requirement, void* state, void* config) {
    ...
    *memory_requirement = sizeof(renderer_system_state);
    ...
    if (!state_ptr->plugin.initialize(&state_ptr->plugin, &renderer_config, &state_ptr->window_render_target_count)) {
        DERROR("Renderer backend failed to initialize. Shutting down.");
        return false;
    }
...
b8 renderer_present(struct frame_data* p_frame_data) {
    ...
    if (!result) {
        DERROR("renderer_present failed. Application shutting down...");
    }
...
renderbuffer* renderer_renderbuffer_get(renderbuffer_type type) {
    ...
    switch (type) {
        ...
        default:
            DERROR("Unsupported buffer type %u", type);
            return 0;
    }
}
...
b8 renderer_geometry_create(geometry* g, u32 vertex_size, u32 vertex_count, const void* vertices, u32 index_size, u32 index_count, const void* indices) {
    if (!g) {
        DERROR("renderer_geometry_create requires a valid pointer to geometry.");
        return false;
    }
    if (!vertex_count || !vertices) {
        DERROR("renderer_geometry_create requires vertex data, and none was supplied. vertex_count=%d, vertices=%p", vertex_count, vertices);
        return false;
    }
...
b8 renderer_geometry_upload(geometry* g) {
    if (!g) {
        DERROR("renderer_geometry_upload requires a valid pointer to geometry.");
        return false;
    }
    renderer_system_state* state_ptr = (renderer_system_state*)systems_manager_get_state(K_SYSTEM_TYPE_RENDERER);
...
b8 renderer_shader_create(shader* s, const shader_config* config, renderpass* pass) {
    ...
    for (u8 i = 0; i < config->stage_count; ++i) {
        ...
        if (!resource_system_load(s->stage_configs[i].filename, RESOURCE_TYPE_TEXT, 0, &text_resource)) {
            DERROR("Unable to read shader file: %s.", s->stage_configs[i].filename);
            return false;
        }
        ...
        if (!platform_watch_file(text_resource.full_path, &s->module_watch_ids[i])) {
            ...
            DWARN("Failed to watch shader source file '%s'.", text_resource.full_path);
        }
...
b8 renderer_shader_set_wireframe(shader* s, b8 wireframe_enabled) {
    ...
    if (!state_ptr->plugin.shader_supports_wireframe(&state_ptr->plugin, s)) {
        ...
        DWARN("Shader does not support wireframe mode: '%s'.", s->name);
        return false;
    }
...
shader_uniform* renderer_shader_uniform_get(shader* s, const char* name) {
    ...
    if (!hashtable_get(&s->uniform_lookup, name, &uniform_index)) {
        DERROR("Shader '%s' does not contain a uniform named '%s'.", s->name, name);
        return false;
    }
...
b8 renderer_renderpass_create(const renderpass_config* config, renderpass* out_renderpass) {
    renderer_system_state* state_ptr = (renderer_system_state*)systems_manager_get_state(K_SYSTEM_TYPE_RENDERER);
    if (!config) {
        DERROR("Renderpass config is required.");
        return false;
    }

    if (config->render_target_count == 0) {
        DERROR("Cannot have a renderpass target count of 0, ya dingus.");
        return false;
    }
...
b8 renderer_renderbuffer_create(const char* name, renderbuffer_type type, u64 total_size, renderbuffer_track_type track_type, renderbuffer* out_buffer) {
    renderer_system_state* state_ptr = (renderer_system_state*)systems_manager_get_state(K_SYSTEM_TYPE_RENDERER);
    if (!out_buffer) {
        DERROR("renderer_renderbuffer_create requires a valid pointer to hold the created buffer.");
        return false;
    }
    ...
    if (!state_ptr->plugin.renderbuffer_internal_create(&state_ptr->plugin, out_buffer)) {
        DFATAL("Unable to create backing buffer for renderbuffer. Application cannot continue.");
        return false;
    }
...
b8 renderer_renderbuffer_bind(renderbuffer* buffer, u64 offset) {
    renderer_system_state* state_ptr = (renderer_system_state*)systems_manager_get_state(K_SYSTEM_TYPE_RENDERER);
    if (!buffer) {
        DERROR("renderer_renderbuffer_bind requires a valid pointer to a buffer.");
        return false;
    }
...
b8 renderer_renderbuffer_resize(renderbuffer* buffer, u64 new_total_size) {
    ...
    if (new_total_size <= buffer->total_size) {
        DERROR("renderer_renderbuffer_resize requires that new size be larger than the old. Not doing this could lead to data loss.");
        return false;
    }
    ...
    if (buffer->track_type == RENDERBUFFER_TRACK_TYPE_FREELIST) {
        ...
        if (!freelist_resize(&buffer->buffer_freelist, &new_memory_requirement, new_block, new_total_size, &old_block)) {
            DERROR("renderer_renderbuffer_resize failed to resize internal free list.");
    ...
    if (result) {
        ...
        DERROR("Failed to resize internal renderbuffer resources.");
    }
    return result;
}
...
b8 renderer_renderbuffer_allocate(renderbuffer* buffer, u64 size, u64* out_offset) {
    if (!buffer || !size || !out_offset) {
        DERROR("renderer_renderbuffer_allocate requires valid buffer, a nonzero size and valid pointer to hold offset.");
        return false;
    }

    if (buffer->track_type == RENDERBUFFER_TRACK_TYPE_NONE) {
        DWARN("renderer_renderbuffer_allocate called on a buffer not using freelists. Offset will not be valid. Call renderer_renderbuffer_load_range instead.");
        *out_offset = 0;
        return true;
    } else if (buffer->track_type == RENDERBUFFER_TRACK_TYPE_LINEAR) {
...
b8 renderer_renderbuffer_free(renderbuffer* buffer, u64 size, u64 offset) {
    if (!buffer || !size) {
        DERROR("renderer_renderbuffer_free requires valid buffer and a nonzero size.");
        return false;
    }

    if (buffer->track_type != RENDERBUFFER_TRACK_TYPE_FREELIST) {
        DWARN("renderer_render_buffer_free called on a buffer not using freelists. Nothing was done.");
        return true;
    }
