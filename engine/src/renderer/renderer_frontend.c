...
b8 renderer_draw_frame(render_packet* packet) {
    ...
    if (state_ptr->backend.begin_frame(&state_ptr->backend, packet->delta_time)) {
        ...
        if (!state_ptr->backend.begin_renderpass(&state_ptr->backend, BUILTIN_RENDERPASS_WORLD)) {
            ERROR("backend.begin_renderpass -> BUILTIN_RENDERPASS_WORLD failed. Application shutting down...");
            return false;
        }
        ...
        if (!state_ptr->backend.end_renderpass(&state_ptr->backend, BUILTIN_RENDERPASS_WORLD)) {
            ERROR("backend.end_renderpass -> BUILTIN_RENDERPASS_WORLD failed. Application shutting down...");
            return false;
        }
        ...
        if (!state_ptr->backend.begin_renderpass(&state_ptr->backend, BUILTIN_RENDERPASS_UI)) {
            ERROR("backend.begin_renderpass -> BUILTIN_RENDERPASS_UI failed. Application shutting down...");
            return false;
        }

        // Update UI global state
        if(!shader_system_use_by_id(state_ptr->ui_shader_id)) {
            ERROR("Failed to use UI shader. Render frame failed.");
            return false;
        }
        ...
        if (!state_ptr->backend.end_renderpass(&state_ptr->backend, BUILTIN_RENDERPASS_UI)) {
            ERROR("backend.end_renderpass -> BUILTIN_RENDERPASS_UI failed. Application shutting down...");
            return false;
        }
        // End UI renderpass
...
b8 renderer_renderpass_id(const char* name, u8* out_renderpass_id) {
    ...
    ERROR("renderer_renderpass_id: No renderpass named '%s'.", name);
