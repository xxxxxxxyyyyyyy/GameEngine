...
b8 rendergraph_pass_create(rendergraph* graph, const char* name, b8 (*create_pfn)(struct rendergraph_pass* self, void* config), void* config, rendergraph_pass* out_pass) {
    ...
    for (u32 i = 0; i < pass_count; ++i) {
        if (strings_equal(graph->passes[i]->name, name)) {
            DERROR("Unable to add pass because a pass named '%s' already exists.", name);
            return false;
        }
    }
    ...
    if (!create_pfn(out_pass, config)) {
        DERROR("Error creating rendergraph pass, See logs for details.");
        return false;
    }
...
b8 rendergraph_pass_source_add(rendergraph* graph, const char* pass_name, const char* source_name, rendergraph_source_type type, rendergraph_source_origin origin) {
    ...
    if (!pass) {
        DERROR("Unable to find a rendergraph pass named '%s'.", pass_name);
        return false;
    }
    ...
    for (u32 i = 0; i < source_count; ++i) {
        if (strings_equal(pass->sources[i].name, source_name)) {
            DERROR("The pass '%s' already has a source named '%s'. Source not added.", pass_name, source_name);
            return false;
        }
    }
...
b8 rendergraph_pass_sink_add(rendergraph* graph, const char* pass_name, const char* sink_name) {
    ...
    for (u32 i = 0; i < sink_count; ++i) {
        if (strings_equal(pass->sinks[i].name, sink_name)) {
            DERROR("The pass '%s' already has a sink named '%s'. Sink not added.", pass_name, sink_name);
            return false;
        }
    }
...
b8 rendergraph_pass_set_sink_linkage(rendergraph* graph, const char* pass_name, const char* sink_name, const char* source_pass_name, const char* source_name) {
    ...
    if (!pass) {
        DERROR("Unable to find a rendergraph target pass named '%s'.", pass_name);
        return false;
    }
    ...
    if (!sink) {
        DERROR("Unable to find sink named '%s' on rendergraph target pass named '%s'.", sink_name, pass_name);
        return false;
    }
    ...
    if (!source_pass_name) {
        ...
        if (!source_pass) {
            DERROR("Unable to find source pass named '%s'.", source_pass_name);
            return false;
        }
    ...
    if (!source) {
        DERROR("Unable to find source named '%s'.", source_name);
        return false;
    }
...
b8 rendergraph_finalize(rendergraph* graph) {
        ...
        if (source->origin == RENDERGRAPH_SOURCE_ORIGIN_GLOBAL) {
            ...
            for (u32 j = 0; j < attachment_count; ++j) {
                if (source->type == RENDERGRAPH_SOURCE_TYPE_RENDER_TARGET_COLOUR) {
                    ...
                    DERROR("Unsupported source type: 0x%x", source->type);
                    return false;
                }
            }
        }
    ...
    if (!backbuffer_first_user) {
        DERROR("Rendergraph configuration error: No reference to global backbuffer source exists.");
        return false;
    }
            ...
            if (source->type == RENDERGRAPH_SOURCE_TYPE_RENDER_TARGET_COLOUR) {
                ...
                if (source->origin == RENDERGRAPH_SOURCE_ORIGIN_OTHER) {
                    ...
                    for (u32 k = 0; k < pass_count; ++k) {
                        ...
                        if (!found) {
                            DWARN("No source found with depth stencil texture available.");
                            break;
                        }
                    }
                    ...
                    if (pass->source_populate) {
                        if (!pass->source_populate(pass, source)) {
                            DERROR("Failed to populate source '%s'.", source->name);
                        }
                    } else {
                        DERROR("Rendergraph pass '%s': source '%s' is set to RENDERGRAPH_SOURCE_ORIGIN_SELF but does not have source_populate defined.");
                        return false;
                    }
                }
            }
    ...
    if (!graph->backbuffer_global_sink.bound_source) {
        DERROR("Unable to link backbuffer_global_sink to a source because no source was found.");
        return false;
    }
    ...
    for (u32 i = 0; i < pass_count; ++i) {
        if (!graph->passes[i]->initialize(graph->passes[i])) {
            DERROR("Error intializing pass. Check logs for more info.");
            return false;
        }
        ...
        if (!regenerate_render_targets(graph, graph->passes[i], 1280, 720)) {
            DERROR("Failed to rengenerate render targets");
            return false;
        }
    }
...
b8 rendergraph_execute_frame(rendergraph* graph, frame_data* p_frame_data) {
    ...
    for (u32 i = 0; i < pass_count; ++i) {
        ...
        if (!graph->passes[i]->execute(graph->passes[i], p_frame_data)) {
            DERROR("Error executing pass. Check logs for additional details.");
            return false;
        }
    }
            ...
            if (attachment->source == RENDER_TARGET_ATTACHMENT_SOURCE_DEFAULT) {
                ...
                if (pass->attachment_textures_regenerate) {
                    if (!pass->attachment_textures_regenerate(pass, width, height)) {
                        DERROR("Failed to regenerate attachment textures for rendergraph pass'%s'.", pass->name);
                    }
                }
                if (!pass->attachment_populate) {
                    DERROR("Attempted to get a self-owned attachment texture for a pass that does not implement attachment_populate.");
                    return false;
                }
