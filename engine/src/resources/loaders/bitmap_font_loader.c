...
#define SUPPORTED_FILETYPE_COUNT 2
    ...
    if (type == BITMAP_FONT_FILE_TYPE_NOT_FOUND) {
        DERROR("Unable to find bitmap font of supported type called '%s'.", name);
        return false;
    }
    ...
    if (!result) {
        DERROR("Failed to process bitmap font file '%s'.", full_file_path);
...
#define VERIFY_LINE(line_type, line_num, expected, actual)                                                                                     \
    if (actual != expected) {                                                                                                                  \
        DERROR("Error in file format reading type '%s', line %u. Expected %d element(s) but read %d.", line_type, line_num, expected, actual); \
        return false;                                                                                                                          \
    }
            ...
            case 'c': {
                ...
                if (line_buf[1] == 'o') {
                    ...
                    if (out_data->page_count > 0) {
                        ...
                        DERROR("Pages is 0, which should not be possible. Font file reading aborted.");
                        return false;
                    }
                    ...
                    if (line_buf[4] == 's') {
                        ...
                        if (out_data->data.glyph_count > 0) {
                            ...
                            DERROR("Glyph count is 0, which should not be possible. Font file reading aborted.");
                            return false;
                        }
                    } else {
                        // Assume 'char' line
...
static b8 read_kbf_file(file_handle* file, bitmap_font_resource_data* data) {
    ...
    if (header.magic_number != RESOURCE_MAGIC && header.resource_type == RESOURCE_TYPE_BITMAP_FONT) {
        DERROR("KBF file header is invalid and cannot be read.");
...
static b8 write_kbf_file(const char* path, bitmap_font_resource_data* data) {
    ...
    if (!filesystem_open(path, FILE_MODE_WRITE, true, &file)) {
        DERROR("Failed to open file for writing: %s", path);
        return false;
    }
