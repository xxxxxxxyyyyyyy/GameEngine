...
b8 image_loader_query_properties(const char *image_name, i32 *out_width, i32 *out_height, i32 *out_channels, u32 *out_mip_levels) {
    ...
    if (!image_base_path) {
        DERROR("Unable to query image base path. Cannot query image properties as a result");
        return false;
    }
    ...
    if (!found) {
        DERROR("Unable to find image named '%s'.", image_name);
        return false;
    }
    ...
    if (result == 0) {
        DERROR("Failed to query image data from memory.");
        final_result = false;
        goto image_loader_query_return;
    }
