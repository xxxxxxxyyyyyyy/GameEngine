    ...
    while (filesystem_read_line(&f, 511, &p, &line_length)) {
        ...
        if (strings_equali(trimmed_var_name, "version")) {
            ...
            if (!string_to_u32(trimmed_value, &resource_data->max_instances)) {
                DERROR("Invalid value for max_instances. Cannot be parsed to u32. Defaulting to &u", resource_data->max_instances);
            }
            ...
            for (u32 sn_idx = 0; sn_idx < count; ++sn_idx) {
                ...
                if (strings_equali(stage_names[sn_idx], "frag") || strings_equali(stage_names[sn_idx], "fragment")) {
                    ...
                    DERROR("shader_loader_load: Invalid file layout. Unrecognized stage '%s'", stage_names[sn_idx]);
                }
            }
            ...
            if (count > 0) {
                ...
                for (u32 i = 0; i < count; ++i) {
                    if (strings_equali(topologies[i], "triangle_list")) {
                        ...
                        DERROR("Unrecognized topology type '%s'. Skipping.", topologies[i]);
                    }
                }
            }
            ...
            if (field_count != 3) {
                ...
                if (array_length < 1) {
                    DWARN("Cannot have an array with a length < 1. Defaulting to 1.");
                    array_length = 1;
                }
                char base_type[100];
                if (is_array) {
                    string_sub(base_type, fields[0], 0, string_index_of(fields[0], '['));
                } else {
                ...
                if (strings_equali(base_type, "f32")) {
                    ...
                    if (strings_equali(base_type, "sampler1d")) {
                        ...
                        DERROR("Error in shader file: Unsupported sampler type '%s' found. %s:%u", fields[0], full_file_path, line_number);
                        return false;
                    }
                    ...
                    if (len <= 6) {
                        DERROR("shader_loader_load: Invalid struct uniform, size is missing. Shader load aborted.");
                        return false;
                    }
                    // u32 diff = len - 6;
                    char struct_size_str[32] = {0};
                    string_sub(struct_size_str, fields[0], 6, -1);
                    u32 struct_size = 0;
                    if (!string_to_u32(struct_size_str, &struct_size)) {
                        DERROR("Unable to parse struct uniform size. Shader load aborted.");
                        return false;
                    }
                    ...
                    DERROR("shader_loader_load: Invalid file layout. Uniform type must be f32, vec2, vec3, vec4, i8, i16, i32, u8, u16, u32 or mat4.");
                    DWARN("Defaulting to f32.");
                    uniform.type = SHADER_UNIFORM_TYPE_FLOAT32;
                    uniform.size = 4;
                }
