...
static b8 terrain_loader_load(struct resource_loader *self, const char *name,
    ...
    while (filesystem_read_line(&f, 511, &p, &line_length)) {
        ...
        if (strings_equali(trimmed_var_name, "version")) {
            if (!string_to_u32(trimmed_value, &version)) {
                DWARN("Format error - invalid file version.");
            }
            // TODO: version
            ...
            if (!string_to_u32(trimmed_var_name + 8, &material_index)) {
                DWARN("Format error: Unable to parse material index.");
            }
            ...
            if (!string_to_u32(trimmed_value, &resource_data->chunk_size)) {
                DWARN("Format error - invalid chunk size. Defaulting to 16.");
    ...
    if (heightmap_file) {
        ...
        if (!resource_system_load(heightmap_file, RESOURCE_TYPE_IMAGE, &params,
            ...
            DERROR("Unable to load heightmap file for terrain. Setting some reasonable defaults.");
        ...
        DWARN("No heightmap was included, using reasonable defaults for terrain generation.");
        resource_data->tile_count_x = resource_data->tile_count_z = 128;
        resource_data->chunk_size = 16;
        resource_data->vertex_data_length = resource_data->tile_count_x * resource_data->tile_count_z;
        resource_data->vertex_datas = darray_reserve(terrain_vertex_data, resource_data->vertex_data_length);
...
static void terrain_loader_unload(struct resource_loader *self,
    ...
    if (!resource_unload(self, resource, MEMORY_TAG_RESOURCE)) {
        DWARN("terrain_loader_unload called with nullptr for self or resource.");
    }
}
