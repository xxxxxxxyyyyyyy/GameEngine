...
void mesh_load_job_success(void* params) {
    ...
    DTRACE("Successfully loaded mesh '%s'.", mesh_params->resource_name);
...
void mesh_load_job_fail(void* params) {
    ...
    DERROR("Failed to load mesh '%s'.", mesh_params->resource_name);
...
b8 mesh_destroy(mesh* m) {
    ...
    if (m->geometries) {
        if (!mesh_unload(m)) {
            DERROR("mesh_destroy - failed to unload mesh.");
            return false;
        }
    }
