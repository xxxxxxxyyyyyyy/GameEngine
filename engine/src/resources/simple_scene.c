...
b8 simple_scene_initialize(simple_scene* scene) {
    ...
    for (u32 i = 0; i < mesh_count; ++i) {
        if (scene->meshes[i].config.parent_name) {
            ...
            if (!parent) {
                DWARN("Mesh '%s' is configured to have a parent called '%s', but the parent does not exist.", scene->meshes[i].config.name, scene->meshes[i].config.parent_name);
                continue;
            }
...
b8 simple_scene_directional_light_add(simple_scene* scene, const char* name, struct directional_light* light) {
    ...
    if (light) {
        if (!light_system_directional_add(light)) {
            DERROR("simple_scene_directional_light_add - failed to add directional light to light system.");
            return false;
        }
    }
...
b8 simple_scene_directional_light_remove(simple_scene* scene, const char* name) {
    ...
    if (!scene->dir_light || !strings_equal(scene->dir_light->name, name)) {
        DWARN("Cannot remove directional light from scene that is not part of the scene.");
        return false;
    }
