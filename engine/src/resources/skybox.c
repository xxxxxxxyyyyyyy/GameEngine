...
b8 skybox_create(skybox_config config, skybox* out_skybox) {
    if (!out_skybox) {
        DERROR("skybox_create requires a valid pointer to out_skybox!");
        return false;
    }
...
b8 skybox_initialize(skybox* sb) {
    if (!sb) {
        DERROR("skybox_initialize requires a valid pointer to sb!");
        return false;
    }
...
b8 skybox_load(skybox* sb) {
    if (!sb) {
        DERROR("skybox_load requires a valid pointer to sb!");
        return false;
    }
...
b8 skybox_unload(skybox* sb) {
    if (!sb) {
        DERROR("skybox_unload requires a valid pointer to sb!");
        return false;
    }
...
void skybox_destroy(skybox* sb) {
    if (!sb) {
        DERROR("skybox_destroy requires a valid pointer to sb!");
        return;
    }
    ...
    if (sb->instance_id != INVALID_ID) {
        ...
        if (!result) {
            DERROR("skybox_destroy() - Failed to successfully unload skybox before destruction.");
        }
    }
