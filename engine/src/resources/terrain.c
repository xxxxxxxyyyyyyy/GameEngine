...
b8 terrain_create(const terrain_config *config, terrain *out_terrain) {
    if (!out_terrain) {
        DERROR("terrain_create requires a valid pointer to out_terrain.");
        return false;
    }
    ...
    if (!config->tile_count_x) {
        DERROR("Tile count x cannot be less than one.");
        return false;
    }

    if (!config->tile_count_z) {
        DERROR("Tile count z cannot be less than one.");
        return false;
    }

    if (!config->chunk_size) {
        DERROR("Chunk size cannot be less than one.");
        return false;
    }
...
void terrain_destroy(terrain *t) {
    ...
    if (t->generation != INVALID_ID) {
        if (!terrain_unload(t)) {
            DERROR("Failed to properly unload terrain before destroying. See logs for details.");
        }
    }
...
b8 terrain_initialize(terrain *t) {
    if (!t) {
        DERROR("terrain_initialize requires a valid pointer to a terrain!");
        return false;
    }
...
b8 terrain_load(terrain *t) {
    if (!t) {
        DERROR("terrain_load requires a valid pointer to a terrain, ya dingus!");
        return false;
    }
    ...
    for (u32 i = 0; i < t->chunk_count; ++i) {
        if (!terrain_chunk_load(t, &t->chunks[i])) {
            ...
            terrain_destroy(t);
            DERROR("Terrain chunk failed to load, thus the terrain cannot be loaded.");
            return false;
        }
    }
...
b8 terrain_chunk_load(terrain *t, terrain_chunk *chunk) {
    ...
    if (!renderer_renderbuffer_allocate(vertex_buffer, total_vertex_size, &chunk->vertex_buffer_offset)) {
        DERROR("Failed to allocate memory for terrain chunk vertex data.");
        return false;
    }
    ...
    if (!renderer_renderbuffer_load_range(vertex_buffer, chunk->vertex_buffer_offset, total_vertex_size, chunk->vertices, false)) {
        DERROR("Failed to upload vertex data for terrain chunk.");
        return false;
    }
    ...
    for (u32 i = 0; i < t->lod_count; ++i) {
        ...
        if (!renderer_renderbuffer_allocate(index_buffer, total_size, &lod->index_buffer_offset)) {
            DERROR("Failed to allocate memory for terrain chunk lod index data.");
            return false;
        }

        // TODO: Passing false here produces a queue wait and should be offloaded to another queue.
        if (!renderer_renderbuffer_load_range(index_buffer, lod->index_buffer_offset, total_size, lod->indices, false)) {
            DERROR("Failed to upload index data for terrain chunk lod.");
            return false;
        }
    }
    ...
    if (!chunk->material) {
        DWARN("Failed to acquire terrain material. Using defualt instead.");
...
b8 terrain_unload(terrain *t) {
    if (!t) {
        DERROR("terrain_unload requires a valid pointer to a terrain.");
        return false;
    }
    ...
    for (u32 i = 0; i < t->chunk_count; ++i) {
        ...
        if (!terrain_chunk_unload(t, chunk)) {
            DERROR("Failed to unload terrain chunk. See logs for details.");
...
b8 terrain_chunk_unload(terrain *t, terrain_chunk *chunk) {
    if (!t || !chunk) {
        DERROR("terrain_chunk_unload requires valid pointers to terrain and chunk to be unloaded.");
        return false;
    }
    ...
    if (chunk->vertices) {
        ...
        if (!renderer_renderbuffer_free(vertex_buffer, sizeof(terrain_vertex) * chunk->total_vertex_count, chunk->vertex_buffer_offset)) {
            DERROR("Error freeing vertex data for terrain chunk. See logs for details.");
        ...
        for (u32 j = 0; j < t->lod_count; ++j) {
            ...
            if (lod->indices) {
                if (!renderer_renderbuffer_free(index_buffer, sizeof(u32) * lod->total_index_count, lod->index_buffer_offset)) {
                    DERROR("Error freeing vertex data for terrain chunk, lod level=%u. See logs for details.", j);
                    has_error = true;  // Flag that an error occurred.
                }
            }
        }
...
static void terrain_chunk_destroy(terrain *t, terrain_chunk *chunk) {
    ...
    if (chunk->generation != INVALID_ID_U16) {
        if (!terrain_chunk_unload(t, chunk)) {
            ...
            DERROR("Failed to unload terrain chunk before destroying it. See logs for details.");
        }
    }
