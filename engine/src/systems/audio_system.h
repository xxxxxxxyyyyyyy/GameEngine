 ...
 */
API b8 audio_system_initialize(u64* memory_requirement, void* state, void* config);
 ...
 */
API void audio_system_shutdown(void* state);

/**
 * @brief Updates the audio system. Should happen once an update cycle.
 */
API b8 audio_system_update(void* state, struct frame_data* p_frame_data);
 ...
 * @return True on success; otherwise false.
 */
API b8 audio_system_listener_orientation_set(vec3 position, vec3 forward, vec3 up);
 ...
 */
API struct audio_file* audio_system_chunk_load(const char* path);
 ...
 */
API struct audio_file* audio_system_stream_load(const char* path);

/**
 * @brief Closes the given sound, releasing all internal resources.
 * @param file A pointer to the sound file to be closed.
 */
API void audio_system_close(struct audio_file* file);
 ...
 */
API void audio_system_master_volume_set(f32 volume);

/**
 * @brief Queries the master volume.
 * @param volume A pointer to hold the volume.
 */
API void audio_system_master_volume_query(f32* out_volume);
 ...
 * @return True on success; otherwise false.
 */
API b8 audio_system_channel_volume_set(i8 channel_id, f32 volume);
 ...
 * @return True on success; otherwise false.
 */
API b8 audio_system_channel_volume_query(i8 channel_id, f32* out_volume);
 ...
 * @return True on success; otherwise false.
 */
API b8 audio_system_channel_play(i8 channel_id, struct audio_file* file, b8 loop);
 ...
 * @return True on success; otherwise false.
 */
API b8 audio_system_channel_emitter_play(i8 channel_id, struct audio_emitter* emitter);
 ...
 * @return True on success; otherwise false.
 */
API void audio_system_channel_stop(i8 channel_id);
API void audio_system_channel_pause(i8 channel_id);
API void audio_system_channel_resume(i8 channel_id);