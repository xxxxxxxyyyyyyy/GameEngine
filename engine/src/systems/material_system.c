...
typedef struct material_system_state {
    ...
    matrix4 directional_light_space[MAX_SHADOW_CASCADE_COUNT];
...
b8 material_system_initialize(u64* memory_requirement, void* state, void* config) {
    ...
    *memory_requirement = struct_requirement + array_requirement + hashtable_requirement;
    ...
    if (!create_default_pbr_material(state_ptr)) {
        DFATAL("Failed to create default PBR material. Application cannot continue.");
        return false;
    }

    if (!create_default_terrain_material(state_ptr)) {
        DFATAL("Failed to create default terrain material. Application cannot continue.");
        return false;
    }
...
static material* material_system_acquire_reference(const char* name, b8 auto_release, b8* needs_creation) {
    ...
    DERROR("material_system_acquire_from_config failed to acquire material '%s'. Null pointer will be returned.", name);
    return 0;
}
...
material* material_system_acquire_terrain_material(const char* material_name, u32 material_count, const char** material_names, b8 auto_release) {
    ...
    if (!m) {
        DERROR("Failed to acquire terrain material '%s'", material_name);
        return 0;
    }
    ...
    if (needs_creation) {
        ...
        for (u32 i = 0; i < material_count; ++i) {
            ...
            if (mat_config->type != MATERIAL_TYPE_PBR) {
                DERROR("Terrain materials must be PBR materials.");
                return false;
            }
        ...
        // One map is needed for the entire material array.
        {
            ...
            if (!map->texture) {
                ...
                DWARN("Unable to load arrayed texture '%s' for material '%s', using default.", m->name, material_name);
                map->texture = texture_system_get_default_terrain_texture();
            }
            if (!renderer_texture_map_resources_acquire(map)) {
                DERROR("Unable to acquire resources for texture map.");
                return false;
            }
        }
        ...
        // Shadow maps can't be configured, so set them up here.
        {
            ...
            if (!assign_map(&m->maps[SAMP_TERRAIN_SHADOW_MAP], &map_config, m->name, texture_system_get_default_diffuse_texture())) {
                DERROR("Failed to assign '%s' texture map for terrain shadow map.", map_config.name);
                return false;
            }
        }
        ...
        // IBL - cubemap for irradiance
        {
            ...
            if (!assign_map(&m->maps[SAMP_TERRAIN_IRRADIANCE_MAP], &map_config, m->name, texture_system_get_default_cube_texture())) {
                DERROR("Failed to assign '%s' texture map for terrain irradiance map.", map_config.name);
                return false;
            }
        }
        ...
        if (!result) {
            DERROR("Failed to acquire renderer resources for terrain material '%s'.", m->name);
        }
...
material* material_system_get_default_pbr(void) {
    ...
    DFATAL("material_system_get_default_pbr called before system is initialized.");
    return 0;
}
...
material* material_system_get_default_terrain(void) {
    ...
    DFATAL("material_system_get_default_terrain called before system is initialized.");
    return 0;
}
...
b8 material_system_apply_global(u32 shader_id, struct frame_data* p_frame_data, const matrix4* projection, const matrix4* view, const vec4* ambient_colour, const vec3* view_position, u32 render_mode) {
...
b8 material_system_apply_local(material* m, const matrix4* model, frame_data* p_frame_data) {
...
b8 material_system_irradiance_set(texture* irradiance_cube_texture) {
    if (irradiance_cube_texture) {
        if (irradiance_cube_texture->type != TEXTURE_TYPE_CUBE) {
            DWARN("material_system_irradiance_set requires parameter irradiance_cube_texture to be a cubemap type texture. Nothing was done.");
            return false;
        }
...
void material_system_directional_light_space_set(matrix4 directional_light_space, u8 index) {
...
static b8 assign_map(texture_map* map, const material_map* config, const char* material_name, texture* default_tex) {
    ...
    if (string_length(config->texture_name) > 0) {
        ...
        if (!map->texture) {
            // Configured, but not found.
            DWARN("Unable to load texture '%s' for material '%s', using default.", config->texture_name, material_name);
    ...
    if (config->type == MATERIAL_TYPE_PBR) {
        ...
        for (u32 i = 0; i < configure_map_count; ++i) {
            ...
            if (strings_equali(config->maps[i].name, "ibl_cube")) {
                ...
                DWARN("Configuration for material '%s' contains a map named '%s', which will be ignored for PBR material types.", config->name, config->maps[i].name);
            }
        }
        ...
        if (!config->shader_name) {
            DERROR("Shader name is required for custom material types. Material '%s' failed to load", m->name);
            return false;
        }
        ...
        for (u32 i = 0; i < prop_count; ++i) {
            if (config->properties[i].size > 0) {
                ...
                switch (config->properties[i].type) {
                    ...
                    default:
                        // TODO: custom size?
                        DWARN("Unable to process shader uniform type %d (index %u) for material '%s'. Skipping.", config->properties[i].type, i, m->name);
                        continue;
                }
        ...
        DERROR("Unknown material type: %d. Material '%s' cannot be loaded.", config->type, m->name);
        return false;
    }
...
static b8 create_default_pbr_material(material_system_state* state) {
    ...
    if (!renderer_shader_instance_resources_acquire(s, &instance_resource_config, &state->default_pbr_material.internal_id)) {
        DFATAL("Failed to acquire renderer resources for default PBR material. Application cannot continue.");
        return false;
    }
...
static b8 create_default_terrain_material(material_system_state* state) {
    ...
    if (!result) {
        DERROR("Failed to acquire renderer resources for default terrain material '%s'.");
    }
