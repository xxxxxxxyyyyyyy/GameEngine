...
b8 render_view_system_register(render_view* view) {
    if (!view) {
        DERROR("render_view_system_register requires a pointer to a valid view.");
        return false;
    }

    if (!view->name || string_length(view->name) < 1) {
        DERROR("render_view_system_register: name is required");
        return false;
    }

    if (view->renderpass_count < 1) {
        DERROR("render_view_system_register - View must have at least one renderpass.");
        return false;
    }
    ...
    if (id != INVALID_ID_U16) {
        DERROR("render_view_system_register - A view named '%s' already exists. A new one will not be registered.", view->name);
        return false;
    }
    ...
    if (id == INVALID_ID_U16) {
        DERROR("render_view_system_register - No available space for a new view. Change system config to account for more.");
        return false;
    }
    ...
    if (!view->on_registered(view)) {
        DERROR("Failed to register view '%s'.", view->name);
            ...
            for (u32 a = 0; a < target->attachment_count; ++a) {
                ...
                if (attachment->source == RENDER_TARGET_ATTACHMENT_SOURCE_DEFAULT) {
                    if (attachment->type == RENDER_TARGET_ATTACHMENT_TYPE_COLOUR) {
                        ...
                        DFATAL("Unsupported attachment type: 0x%x", attachment->type);
                        continue;
                    }
                } else if (attachment->source == RENDER_TARGET_ATTACHMENT_SOURCE_VIEW) {
                    if (!view->attachment_target_regenerate) {
                        DFATAL("RENDER_TARGET_ATTACHMENT_SOURCE_VIEW configured for an attachment whose view does not support this operation.");
                        continue;
                    } else {
                        if (!view->attachment_target_regenerate(view, r, attachment)) {
                            DERROR("View failed to regenerate attachment target for attachment type: 0x%x", attachment->type);
                        }
                    }
                }
            }
