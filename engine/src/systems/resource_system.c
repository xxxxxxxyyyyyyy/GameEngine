...
b8 resource_system_initialize(u64* memory_requirement, void* state, void* config) {
    ...
    *memory_requirement = sizeof(resource_system_state) + (sizeof(resource_loader) * typed_config->max_loader_count);
    ...
    DINFO("Resource system initialized with base path '%s'.", typed_config->asset_base_path);
        ...
        for (u32 i = 0; i < count; ++i) {
            ...
            if (l->id != INVALID_ID) {
                if (l->type == loader.type) {
                    DERROR("resource_system_loader_register - Loader of type %d already exists and will not be registered.", loader.type);
                    return false;
                } else if (loader.custom_type && string_length(loader.custom_type) > 0 && strings_equali(l->custom_type, loader.custom_type)) {
                    DERROR("resource_system_loader_register - Loader of custom type %s already exists and will not be registered.", loader.custom_type);
                    return false;
                }
            }
        }
