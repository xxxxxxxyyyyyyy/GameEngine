            ...
            for (u32 w = 0; w < s->shader_stage_count; ++w) {
                if (s->module_watch_ids[w] == file_watch_id) {
                    if (!shader_system_reload(s)) {
                        DWARN("Shader hot-reload failed for shader '%s'. See logs for details.", s->name);
...
b8 shader_system_create(renderpass* pass, const shader_config* config) {
    ...
    for (u32 i = 0; i < config->attribute_count; ++i) {
        ...
        if (!internal_attribute_add(out_shader, ac)) {
            DERROR("Failed to add attribute '%s' to shader '%s'.", ac->name, config->name);
            return false;
        }
    }
    ...
    for (u32 i = 0; i < config->uniform_count; ++i) {
        ...
        if (uniform_type_is_sampler(uc->type)) {
            if (!internal_sampler_add(out_shader, uc)) {
                DERROR("Failed to add sampler '%s' to shader '%s'.", uc->name, config->name);
                return false;
            }
        } else {
            if (!internal_uniform_add(out_shader, uc, INVALID_ID)) {
                DERROR("Failed to add uniform '%s' to shader '%s'.", uc->name, config->name);
                return false;
            }
        }
    }
...
u32 shader_system_get_id(const char* shader_name) {
    ...
    // DTRACE("Got id %u for shader named '%s'.", shader_id, shader_name);
    return shader_id;
}

shader* shader_system_get_by_id(u32 shader_id) {
    if (shader_id == INVALID_ID) {
        DERROR("shader_system_get_by_id was passed INVALID_ID. Null will be returned.");
        return 0;
    }
    if (state_ptr->shaders[shader_id].id == INVALID_ID) {
        DERROR("shader_system_get_by_id was passed an invalid id (%u. Null will be returned.", shader_id);
        return 0;
    }
    if (shader_id >= state_ptr->config.max_shader_count) {
        DERROR("shader_system_get_by_id was passed an id (%u) out of range (0-%u). Null will be returned.", shader_id, state_ptr->config.max_shader_count);
        return 0;
    }
