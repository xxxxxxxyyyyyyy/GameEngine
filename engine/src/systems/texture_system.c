...
texture* texture_system_acquire(const char* name, b8 auto_release) {
    ...
    if (strings_equali(name, DEFAULT_DIFFUSE_TEXTURE_NAME)) {
        DWARN("texture_system_acquire called for default diffuse texture. Use texture_system_get_default_diffuse_texture for texture 'default_DIFF'.");
        return &state_ptr->default_diffuse_texture;
    }

    if (strings_equali(name, DEFAULT_SPECULAR_TEXTURE_NAME)) {
        DWARN("texture_system_acquire called for default texture. Use texture_system_get_default_specular_texture for texture 'default_SPEC'.");
        return &state_ptr->default_specular_texture;
    }

    if (strings_equali(name, DEFAULT_NORMAL_TEXTURE_NAME)) {
        DWARN("texture_system_acquire called for default texture. Use texture_system_get_default_normal_texture for texture 'default_NORM'.");
    ...
    if (!process_texture_reference(name, 1, auto_release, &id, &needs_creation)) {
        DERROR("texture_system_acquire failed to obtain a new texture id.");
        return 0;
    }
    ...
    if (needs_creation) {
        if (!create_texture(t, TEXTURE_TYPE_2D, 0, 0, 0, 1, 0, false, false)) {
            DERROR("texture_system_acquire failed to create new texture.");
            return 0;
        }
    }
...
texture* texture_system_acquire_cube(const char* name, b8 auto_release) {
    ...
    // TODO: Check against other default texture names?
    if (strings_equali(name, DEFAULT_TEXTURE_NAME)) {
        DWARN("texture_system_acquire_cube called for default texture. Use texture_system_get_default_texture for texture 'default'.");
        return &state_ptr->default_texture;
    }
    ...
    // NOTE: Increments reference count, or creates new entry.
    if (!process_texture_reference(name, 1, auto_release, &id, &needs_creation)) {
        DERROR("texture_system_acquire_cube failed to obtain a new texture id.");
        return 0;
    }
...
texture* texture_system_acquire_writeable_arrayed(const char* name, u32 width, u32 height, u8 channel_count, b8 has_transparency, texture_type type, u16 array_size) {
    ...
    if (!process_texture_reference(name, 1, false, &id, &needs_creation)) {
        DERROR("texture_system_acquire_writeable_arrayed failed to obtain a new texture id.");
        return 0;
    }
...
texture* texture_system_acquire_textures_as_arrayed(const char* name, u32 layer_count, const char** layer_texture_names, b8 auto_release) {
    if (layer_count < 1) {
        DERROR("Must contain at least one layer.");
        return 0;
    }
    ...
    u32 id = INVALID_ID;
    if (!process_texture_reference(name, 1, auto_release, &id, &needs_creation)) {
        DERROR("texture_system_acquire_textures_as_arrayed failed to obtain a new texture id.");
        return 0;
    }
    ...
    if (needs_creation) {
        if (!create_texture(t, TEXTURE_TYPE_2D_ARRAY, 0, 0, 0, layer_count, layer_texture_names, false, false)) {
            DERROR("texture_system_acquire_textures_as_arrayed failed to create new texture.");
            return 0;
        }
    }
...
void texture_system_release(const char* name) {
    ...
    if (!process_texture_reference(name, -1, false, &id, &needs_creation)) {
        DERROR("texture_system_release failed to release texture '%s' properly.", name);
    }
}
...
void texture_system_wrap_internal(const char* name, u32 width, u32 height, u8 channel_count, b8 has_transparency, b8 is_writeable, b8 register_texture, void* internal_data, texture* out_texture) {
    ...
    if (register_texture) {
        ...
        if (!process_texture_reference(name, 1, false, &id, &needs_creation)) {
            DERROR("texture_system_wrap_internal failed to obtain a new texture id.");
            return;
        }
        ...
        if (out_texture) {
            ...
            // DTRACE("texture_system_wrap_internal created texture '%s', but not registering, resulting in an allocation. It is up to the caller to free this memory.", name);
        }
    }
...
b8 texture_system_resize(texture* t, u32 width, u32 height, b8 regenerate_internal_data) {
    if (t) {
        if (!(t->flags & TEXTURE_FLAG_IS_WRITEABLE)) {
            DWARN("texture_system_resize should not be called on textures that are not writeable.");
            return false;
        }
        t->width = width;
        t->height = height;
...
#define RETURN_TEXT_PTR_OR_NULL(texture, func_name)                                              \
    ...
    DERROR("%s called before texture system initialization! Null pointer returned.", func_name); \
    return 0;
...
static b8 create_default_textures(texture_system_state* state) {
    ...
    // Diffuse texture.
    DTRACE("Creating default diffuse texture...");
    ...
    // Specular texture.
    DTRACE("Creating default specular texture...");
    ...
    // Normal texture.
    DTRACE("Creating default normal texture...");
    ...
    // Combined texture
    DTRACE("Creating default combined (metallic, roughness, AO) texture...");
    ...
    // Cube texture.
    DTRACE("Creating default cube texture...");
...
static b8 load_cube_textures(const char texture_names[6][TEXTURE_NAME_MAX_LENGTH], texture* t) {
    ...
    for (u8 i = 0; i < 6; ++i) {
        ...
        if (!resource_system_load(texture_names[i], RESOURCE_TYPE_IMAGE, &params, &img_resource)) {
            DERROR("load_cube_textures() - Failed to load image resource for texture '%s'", texture_names[i]);
            return false;
        }
        ...
        if (!pixels) {
            ...
            if (t->width != resource_data->width || t->height != resource_data->height || t->channel_count != resource_data->channel_count) {
                DERROR("load_cube_textures - All textures must be the same resolution and bit depth.");
                kfree(pixels, sizeof(u8) * image_size * 6, MEMORY_TAG_ARRAY);
                pixels = 0;
                return false;
            }
...
static void texture_load_job_success(void* params) {
    ...
    *texture_params->out_texture = texture_params->temp_texture;
    ...
    DTRACE("Successfully loaded texture '%s'.", texture_params->resource_name);
...
static void texture_load_job_fail(void* params) {
    ...
    DERROR("Failed to load texture '%s'.", texture_params->resource_name);
...
static void texture_load_layered_job_success(void* result) {
    ...
    *typed_result->out_texture = typed_result->temp_texture;
    ...
    if (typed_result->name) {
        DTRACE("Successfully loaded layered texture '%s'.", typed_result->name);
        string_free(typed_result->name);
        typed_result->name = 0;
    } else {
        DTRACE("Successfully loaded layered texture.");
    }
...
static void texture_load_layered_job_fail(void* result_data) {
    ...
    switch (typed_result->result_code) {
        case TEXTURE_LOAD_JOB_CODE_RESOURCE_LOAD_FAILED:
            DERROR("Layered texture load failed to load one or more resources.");
            break;
        case TEXTURE_LOAD_JOB_CODE_FIRST_QUERY_FAILED:
            DERROR("Failed to query properties for first layer image. Unable to create arrayed texture.");
            break;
        case TEXTURE_LOAD_JOB_CODE_RESOURCE_DIMENSION_MISMATCH:
            DERROR("Failed to load the layered image because at least one layer's texture is the wrong size.");
            break;
        default:
            DERROR("Layered texture load failed for an unknown reason.");
            break;
    }
}
...
static b8 texture_load_layered_job_start(void* params, void* result_data) {
    ...
    if (!threadpool_create(load_params->layer_count, &pool)) {
        DERROR("Failed to create threadpool. See logs for details.");
        return false;
    }
    ...
    for (; layer < load_params->layer_count; ++layer) {
        ...
        if (!worker_thread_add(&pool.threads[layer], layered_texture_do_work, info)) {
            DERROR("Failed to add work to worker thread.");
            return false;
        }
    }
    ...
    if (load_result) {
        ...
        DERROR("At least one texture layer failed to load. See logs for details.");
...
static b8 load_texture(const char* texture_name, texture* t, const char** layer_names) {
    if (t->type == TEXTURE_TYPE_2D) {
        ...
        DERROR("Texture system attempted to load unknown texture type: %u", t->type);
        return false;
    }
    return true;
}
...
static b8 create_texture(texture* t, texture_type type, u32 width, u32 height, u8 channel_count, u16 array_size, const char** layer_texture_names, b8 is_writeable, b8 skip_load) {
    ...
    if (skip_load) {
        ...
        // DTRACE("Load skipped for texture '%s'. This is expected behaviour.");
    } else {
        switch (t->type) {
            case TEXTURE_TYPE_CUBE: {
                ...
                if (!load_cube_textures(texture_names, t)) {
                    DERROR("Failed to load cube texture '%s'.", t->name);
                    return false;
                }
            } break;
            ...
            case TEXTURE_TYPE_2D_ARRAY: {
                if (!load_texture(t->name, t, layer_texture_names)) {
                    DERROR("Failed to load texture '%s'.", t->name);
                    return false;
                }
            } break;
            default: {
                DERROR("Unrecognized texture type %u. Cannot process texture reference.", t->type);
                return false;
            }
        }
    }
...
static b8 process_texture_reference(const char* name, i8 reference_diff, b8 auto_release, u32* out_texture_id, b8* needs_creation) {
    ...
    *needs_creation = false;
    if (state_ptr) {
            ...
            if (ref.reference_count == 0 && reference_diff > 0) {
                if (reference_diff > 0) {
                    ...
                    if (ref.auto_release) {
                        ...
                        DWARN("Tried to release a texture where autorelease=false, but references was already 0.");
            ...
            if (reference_diff < 0) {
                ...
                if (ref.handle == INVALID_ID) {
                    ...
                    if (*out_texture_id == INVALID_ID) {
                        DFATAL("process_texture_reference - Texture system cannot hold anymore textures. Adjust configuration to allow more.");
                        return false;
                    } else {
        ...
        DERROR("process_texture_reference failed to acquire id for name '%s'. INVALID_ID returned.", name);
        return false;
    }

    DERROR("process_texture_reference called before texture system is initialized.");
    return false;
}