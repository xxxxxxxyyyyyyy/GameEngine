...
static b8 oal_plugin_stream_music_data(audio_plugin* plugin, ALuint buffer, audio_file* audio) {
    ...
    if (size == INVALID_ID_U64) {
        DERROR("Error streaming data. Check logs for more info.");
        return false;
    }
...
static b8 oal_plugin_stream_update(audio_plugin* plugin, audio_file* audio, audio_plugin_source* source) {
    ...
    if (source_state != AL_PLAYING) {
        DTRACE("Stream update, play needed for source id: %u", source->id);
...
static u32 source_work_thread(void* params) {
    ...
    DDEBUG("Audio source thread starting...");
    ...
    DDEBUG("Audio source thread shutting down.");
    return 0;
}
...
b8 oal_plugin_initialize(struct audio_plugin* plugin, audio_plugin_config config) {
    if (plugin) {
        ...
        if (plugin->internal_state->config.max_sources < 1) {
            DWARN("Audio plugin config.max_sources was configured as 0. Defaulting to 8.");
            plugin->internal_state->config.max_sources = 8;
        }
        if (plugin->internal_state->config.max_buffers < 20) {
            DWARN("Audio plugin config.max_buffers was configured to be less than 20, the recommended minimum. Defaulting to 256.");
        ...
        if (!plugin->internal_state->device) {
            DERROR("Unable to obtain OpenAL device. Plugin initialize failed.");
            return false;
        } else {
            DINFO("OpenAL Device acquired.");
        }
        ...
        for (u32 i = 0; i < config.max_sources; ++i) {
            if (!oal_plugin_source_create(plugin, &plugin->internal_state->sources[i])) {
                DERROR("Unable to create audio source in OpenAL plugin.");
                return false;
            }
        }
        ...
        DINFO("OpenAL plugin intialized");

        return true;
    }

    DERROR("oal_plugin_initialize requires a valid pointer to a plugin.");
    return false;
}
...
b8 oal_plugin_listener_position_query(struct audio_plugin* plugin, vec3* out_position) {
    if (!plugin || !out_position) {
        DERROR("oal_plugin_listener_position_query requires valid pointers to a plugin and out_position.");
        return false;
    }
...
b8 oal_plugin_listener_position_set(struct audio_plugin* plugin, vec3 position) {
    if (!plugin) {
        DERROR("oal_plugin_listener_position_set requires a valid pointer to a plugin.");
        return false;
    }
...
b8 oal_plugin_listener_orientation_query(struct audio_plugin* plugin, vec3* out_forward, vec3* out_up) {
    if (!plugin || !out_forward || !out_up) {
        DERROR("oal_plugin_listener_orientation_query requires valid pointers to a plugin, out_forward and out_up.");
        return false;
    }
...
b8 oal_plugin_listener_orientation_set(struct audio_plugin* plugin, vec3 forward, vec3 up) {
    if (!plugin) {
        DERROR("oal_plugin_listener_orientation_set requires a valid pointer to a plugin.");
        return false;
    }
...
static b8 oal_plugin_source_create(struct audio_plugin* plugin, audio_plugin_source* out_source) {
    if (!plugin || !out_source) {
        DERROR("oal_plugin_source_create requires valid pointers to a plugin and out_source.");
        return false;
    }

    alGenSources((ALuint)1, &out_source->id);
    if (!oal_plugin_check_error()) {
        DERROR("Failed to create source.");
        return false;
    }

    if (!source_set_defaults(plugin, out_source, true)) {
        DERROR("Failed to set source defaults, and thus failed to create source.");
    }
...
static void clear_buffer(struct audio_plugin* plugin, u32* buf_ptr, u32 amount) {
    ...
    DWARN("Buffer could not be cleared.");
}
...
static u32 oal_plugin_find_free_buffer(struct audio_plugin* plugin) {
    if (plugin) {
        ...
        if (free_count == 0) {
            DINFO("oal_plugin_find_free_buffer() - no free buffers, attempting to free an existing one.");
            if (!oal_plugin_check_error()) {
                return false;
            }
        ...
        if (free_count < 1) {
            DERROR("Could not find or clear a buffer. This means too many things are being played at once.");
            return INVALID_ID;
        }
        ...
        free_count = darray_length(plugin->internal_state->free_buffers);
        DTRACE("Found free buffer id %u", out_buffer_id);
        DDEBUG("There are now %u free buffers remaining.", free_count);
...
b8 oal_plugin_source_gain_query(struct audio_plugin* plugin, u32 source_index, f32* out_gain) {
    ...
    DERROR("Plugin pointer invalid or source id is invalid: %u.", source_index);
    return false;
}
...
static b8 oal_plugin_check_error(void) {
    ...
    if (error != AL_NO_ERROR) {
        DERROR("OpenAL error %u: '%s'", error, oal_plugin_error_str(error));
        return false;
    }
    return true;
}
...
struct audio_file* oal_plugin_stream_load(struct audio_plugin* plugin, const char* name) {
    ...
    if (!plugin || !name) {
        DERROR("oal_plugin_open_music_file requires a valid pointer to plugin and name");
        return 0;
    }
    ...
    if (!resource_system_load(name, RESOURCE_TYPE_AUDIO, &params, &audio_resource)) {
        DERROR("Failed to open audio resource. Load failed.");
        return 0;
    }
    ...
    for (u32 i = 0; i < OAL_PLUGIN_MUSIC_BUFFER_COUNT; ++i) {
        ...
        if (out_file->plugin_data->buffers[i] == INVALID_ID) {
            DERROR("Unable to open music file due to no buffers being available.");
            return 0;
        }
    }
    oal_plugin_check_error();
...
struct audio_file* oal_plugin_chunk_load(struct audio_plugin* plugin, const char* name) {
    ...
    if (!plugin || !name) {
        DERROR("oal_plugin_open_sound_file requires valid pointers to plugin and name.");
        return 0;
    }
    ...
    if (out_file->plugin_data->buffer == INVALID_ID) {
        ...
        DERROR("Unable to open audio file due to no buffers being available.");
        return false;
    }
    oal_plugin_check_error();
...
void oal_plugin_audio_file_close(struct audio_plugin* plugin, struct audio_file* file) {
    if (!plugin || !file) {
        DERROR("oal_plugin_audio_file_close requires valid pointers to plugin and file");
        return;
    }
...
b8 oal_plugin_play_on_source(struct audio_plugin* plugin, struct audio_file* file, i8 source_index) {
    ...
    DTRACE("Play on source %d", source_index);
    ...
    if (file->type == AUDIO_FILE_TYPE_SOUND_EFFECT) {
        ...
        for (u32 i = 0; i < OAL_PLUGIN_MUSIC_BUFFER_COUNT; ++i) {
            if (!oal_plugin_stream_music_data(plugin, file->plugin_data->buffers[i], file)) {
                DERROR("Failed to stream data to buffer &u in music file. File load failed.", i);
                result = false;
                break;
            }
        }
