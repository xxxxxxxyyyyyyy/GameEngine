...
#include "../../thirdlib/stb_vorbis.h"
// Loading mp3 files.
#define MINIMP3_IMPLEMENTATION
#include "../../thirdlib/minimp3_ex.h"
...
static u64 audio_file_load_samples(struct audio_file* audio, u32 chunk_size, i32 count) {
    ...
    DERROR("Error loading samples: Unknown file type.");
...
static void* audio_file_stream_buffer_data(struct audio_file* audio) {
    if (audio->internal_data->vorbis) {
        ...
        DERROR("Error streaming audio dta: Unknown file type. Null is returned.");
        return 0;
    }
}
void audio_file_rewind(struct audio_file* audio) {
    if (audio) {
        if (audio->internal_data->vorbis) {
            ...
            DERROR("Error rewinding audio file: unknown type.");
            return;
        }
    }
}
...
static b8 audio_loader_load(struct resource_loader* self, const char* name, void* params, resource* out_resource) {
    ...
    if (string_index_of_str(".ogg", full_file_path) != -1) {
        DTRACE("Processing OGG music file '%s'...", full_file_path);
        i32 ogg_error = 0;
        ...
        if (!resource_data->internal_data->vorbis) {
            ...
            DERROR("Failed to load vorbis file with error: %u", ogg_error);
            return false;
        }
        ...
        if (resource_data->type == AUDIO_FILE_TYPE_MUSIC_STREAM) {
            ...
            if (read_samples != (i64)length_samples) {
                DWARN("Read/length mismatch while reading ogg file. This might cause playback issues.");
            }
        ...
        DTRACE("Processing MP3 file '%s'...", full_file_path);
        ...
        DDEBUG("mp3 freq: %dHz, avg kbit/s rate: %u", info->hz, info->avg_bitrate_kbps);
        resource_data->channels = info->channels;
        ...
        DERROR("Unsupported audio file type.");
        return false;
    }
