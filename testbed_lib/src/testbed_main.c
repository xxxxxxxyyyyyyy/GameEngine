...
static b8 game_on_drag(u16 code, void* sender, void* listener_inst, event_context context) {
    ...
    if (drag_button == BUTTON_LEFT) {
        ...
        matrix4 projection_matrix = mat4_perspective(deg_to_rad(45.0f), (f32)state->width / state->height, 0.1f, 4000.0f);
...
b8 configure_render_views(application_config* config) {
    ...
    // Skybox view
    {
        ...
        if (!renderer_renderpass_create(&skybox_pass, &skybox_view.passes[0])) {
            DERROR("Skybox view - Failed to create renderpass '%s'", skybox_view.passes[0].name);
            return false;
        }
    ...
    // World view.
    {
        ...
        if (!renderer_renderpass_create(&world_pass, &world_view.passes[0])) {
            DERROR("World view - Failed to create renderpass '%s'", world_view.passes[0].name);
            return false;
        }
