...
b8 application_boot(struct application* game_inst) {
    ...
    if (!configure_render_views(config)) {
        DERROR("Failed to configure renderer views. Aborting application.");
        return false;
    } */
...
b8 application_initialize(struct application* game_inst) {
    ...
    if (!viewport_create(world_vp_rect2, 0.015f, -4000.0f, 4000.0f, RENDERER_PROJECTION_MATRIX_TYPE_ORTHOGRAPHIC_CENTERED, &state->world_viewport2)) {
        DERROR("Failed to create wireframe viewport. Cannot start application.");
        return false;
    }
    ...
    if (state->main_scene.state >= SIMPLE_SCENE_STATE_LOADED) {
        ...
        if (state->p_light_1) {
            state->p_light_1->data.colour = (vec4){
                CLAMP(ksin(p_frame_data->total_time) * 0.75f + 0.5f, 0.0f, 1.0f),
                CLAMP(ksin(p_frame_data->total_time - (K_2PI / 3)) * 0.75f + 0.5f, 0.0f, 1.0f),
                CLAMP(ksin(p_frame_data->total_time - (K_4PI / 3)) * 0.75f + 0.5f, 0.0f, 1.0f),
                1.0f};
