...
b8 render_view_world_on_registered(struct render_view* self) {
    if (self) {
        ...
        if (!resource_system_load(material_shader_name, RESOURCE_TYPE_SHADER, 0, &material_config_resource)) {
            DERROR("Failed to load builtin material shader resource.");
            return false;
        }
        ...
        if (!shader_system_create(&self->passes[1], config)) {
            DERROR("Failed to load builtin material shader.");
            return false;
        }
        ...
        if (!resource_system_load(skybox_shader_name, RESOURCE_TYPE_SHADER, 0, &skybox_shader_config_resource)) {
            DERROR("Failed to load builtin skybox shader.");
            return false;
        }
        ...
        if (!resource_system_load(terrain_shader_name, RESOURCE_TYPE_SHADER, 0, &terrain_shader_config_resource)) {
            DERROR("Failed to load builtin terrain shader resource.");
            return false;
        }
        ...
        if (!shader_system_create(&self->passes[1], terrain_shader_config)) {
            DERROR("Failed to load builtin terrain shader.");
            return false;
        }
        ...
        if (!resource_system_load(colour3d_shader_name, RESOURCE_TYPE_SHADER, 0, &colour3d_shader_config_resource)) {
            DERROR("Failed to load builtin colour3d shader resource.");
            return false;
        }
        ...
        if (!shader_system_create(&self->passes[1], colour3d_shader_config)) {
            DERROR("Failed to load builtin colour3d shader.");
            return false;
        }
...
b8 render_view_world_on_render(const struct render_view* self, const struct render_view_packet* packet, const struct frame_data* p_frame_data) {
    ...
    // Skybox renderpass.
    {
        ...
        if (!renderer_renderpass_begin(pass, &pass->targets[p_frame_data->render_target_index])) {
            DERROR("render_view_world_on_render skybox pass failed to start.");
            return false;
        }
        ...
        if (packet->skybox_data.sb) {
            ...
            matrix4 view_matrix = packet->view_matrix;
        ...
        if (!renderer_renderpass_end(pass)) {
            DERROR("render_view_world_on_render skybox pass failed to end.");
            return false;
        }
    }
    ...
    // World renderpass.
    {
        ...
        if (!renderer_renderpass_begin(pass, &pass->targets[p_frame_data->render_target_index])) {
            DERROR("render_view_world_on_render world pass failed to start.");
            return false;
        }
        ...
        if (!renderer_renderpass_end(pass)) {
            DERROR("render_view_world_on_render world pass failed to end.");
            return false;
        }
    }
