...
static void acquire_shader_instances(const struct render_view* self) {
    ...
    if (!renderer_shader_instance_resources_acquire(data->ui_shader_info.s, 0, 0, &instance)) {
        DFATAL("render_view_pick failed to acquire UI shader resources.");
        return;
    }
    // World shader
    if (!renderer_shader_instance_resources_acquire(data->world_shader_info.s, 0, 0, &instance)) {
        DFATAL("render_view_pick failed to acquire World shader resources.");
        return;
    }
    // Terrain shader
    if (!renderer_shader_instance_resources_acquire(data->terrain_shader_info.s, 0, 0, &instance)) {
        DFATAL("render_view_pick failed to acquire Terrain shader resources.");
        return;
    }
    data->instance_count++;
...
b8 render_view_pick_on_registered(struct render_view* self) {
    if (self) {
        ...
        if (!resource_system_load(ui_shader_name, RESOURCE_TYPE_SHADER, 0, &config_resource)) {
            DERROR("Failed to load builtin UI Pick shader.");
            return false;
        }
        ...
        if (!resource_system_load(world_shader_name, RESOURCE_TYPE_SHADER, 0, &config_resource)) {
            DERROR("Failed to load builtin World Pick shader.");
            return false;
        }
        ...
        if (!resource_system_load(terrain_shader_name, RESOURCE_TYPE_SHADER, 0, &config_resource)) {
            DERROR("Failed to load builtin Terrain Pick shader.");
            return false;
        }
        config = (shader_config*)config_resource.data;
        ...
        if (!event_register(EVENT_CODE_MOUSE_MOVED, self, on_mouse_moved)) {
            DERROR("Unable to listen for mouse move event, creation failed.");
            return false;
        }
    ...
    DERROR("render_view_pick_on_create - Requires a valid pointer to a view.");
    return false;
}
...
b8 render_view_pick_on_packet_build(const struct render_view* self, struct frame_data* p_frame_data, struct viewport* v, struct camera* c, void* data, struct render_view_packet* out_packet) {
    if (!self || !data || !out_packet) {
        DWARN("render_view_pick_on_packet_build requires valid pointer to view, packet, and data.");
        return false;
    }
...
b8 render_view_pick_on_render(const struct render_view* self, const struct render_view_packet* packet, struct frame_data* p_frame_data) {
    ...
    if (p_frame_data->render_target_index == 0) {
        ...
        if (!shader_system_use_by_id(data->world_shader_info.s->id)) {
            DERROR("Failed to use world pick shader. Render frame failed.");
            return false;
        }
        ...
        for (u32 i = 0; i < world_geometry_count; ++i) {
            ...
            if (!shader_system_uniform_set_by_index(data->world_shader_info.id_colour_location, &id_colour)) {
                DERROR("Failed to apply id colour uniform.");
                return false;
            }
            ...
            if (!shader_system_uniform_set_by_index(data->world_shader_info.model_location, &geo->model)) {
                DERROR("Failed to apply model matrix for world geometry.");
            }
        ...
        if (!shader_system_use_by_id(data->terrain_shader_info.s->id)) {
            DERROR("Failed to use terrain pick shader. Render frame failed.");
            return false;
        }
        ...
        for (u32 i = 0; i < terrain_geometry_count; ++i) {
            ...
            if (!shader_system_uniform_set_by_index(data->terrain_shader_info.model_location, &geo->model)) {
                DERROR("Failed to apply model matrix for terrain geometry.");
            }
        ...
        if (!renderer_renderpass_end(pass)) {
            DERROR("render_view_pick_on_render pass index %u failed to end.", p);
            return false;
        }
        ...
        if (!renderer_renderpass_begin(pass, &pass->targets[p_frame_data->render_target_index])) {
            DERROR("render_view_pick_on_render pass index %u failed to start.", p);
            return false;
        }
        ...
        DFATAL("Cannot use pick pass without it being split into UI/World first due to viewport changes.");
        ...
        for (u32 i = 0; i < packet_data->text_count; ++i) {
            ...
            // Apply the locals
            matrix4 model = transform_world_get(&text->transform);
    ...
    // Clamp to image size
    u16 x_coord = CLAMP(data->mouse_x, 0, self->width - 1);
    u16 y_coord = CLAMP(data->mouse_y, 0, self->height - 1);
    ...
    event_execute(EVENT_CODE_OBJECT_HOVER_ID_CHANGED, 0, context);
...
b8 render_view_pick_attachment_target_regenerate(struct render_view* self, u32 pass_index, struct render_target_attachment* attachment) {
    ...
    if (attachment->type == RENDER_TARGET_ATTACHMENT_TYPE_COLOUR) {
        ...
        DERROR("Unsupported attachment type 0x%x.", attachment->type);
        return false;
    }
