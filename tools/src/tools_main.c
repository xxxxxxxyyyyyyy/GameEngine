...
i32 process_shaders(i32 argc, char** argv) {
    ...
    for (u32 i = 2; i < argc; ++i) {
        ...
        if (!sdk_path) {
            DERROR("Environment variable VULKAN_SDK not found. Check your Vulkan installation.");
            return -4;
        }
        ...
        // Some output.
        DINFO("Processing %s -> %s...", argv[i], out_filename);
        ...
        if (retcode != 0) {
            DERROR("Error compiling shader. See logs. Aborting process.");
            return -5;
        }
    }

    DINFO("Successfully processed all shaders.");
    return 0;
}
...
#endif
    DINFO(
        "DOD Game Engine Tools, Copyright 2021-2022 Travis Vroman.\n\
