...
#include "../thirdlib/stb_image_write.h"
...
i32 main(i32 argc, char** argv) {
    ...
    if (argc < 2) {
        DERROR("tools requires at least one argument.");
        print_help();
        return -1;
    }
    ...
    if (strings_equali(argv[1], "combine") || strings_equali(argv[1], "cmaps")) {
        ...
        DERROR("Unrecognized argument '%s'.", argv[1]);
        print_help();
        return -2;
    }
...
i32 combine_texture_maps(i32 argc, char** argv) {
    if (argc < 3) {
        DERROR("Build shaders mode requires at least one additional argument.");
        return -3;
    }
    ...
    for (u32 i = 2; i < argc; ++i) {
        ...
        if (strings_equali(parts[0], "metallic")) {
            ...
            DERROR("Unrecognized map type '%s'", parts[0]);
            return -5;
        }
    }
    if (out_file_path[0] == 0) {
        DERROR("parameter outfile is required. Usage: outfile=[filename]");
        return -4;
    }
    ...
    for (u32 i = 0; i < MAP_TYPE_MAX; ++i) {
        ...
        if (!maps[i].data) {
            DFATAL("Failed to load file '%s'", maps[i].file_path);
            return -6;
        }
    }
        ...
        if (maps[i].file_path) {
            if (width == -1 || height == -1) {
                ...
                if (maps[i].width != width || maps[i].height != height) {
                    DERROR("All texture maps must be the same width and height.");
                    return -7;
                }
            }
        }
    ...
    if (width == -1 || height == -1) {
        DERROR("Unable to obtain width and height - no textures set?");
        return -8;
    }
    for (u32 i = 0; i < MAP_TYPE_MAX; ++i) {
        if (!maps[i].file_path) {
    ...
    if (!stbi_write_png(out_file_path, width, height, 4, target_buffer, 4 * width)) {
        DERROR("Error writing outfile.");
        return -9;
    }

    DINFO("Successfully processed all maps.");
    return 0;
}
