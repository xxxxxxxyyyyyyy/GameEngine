...
void *vulkan_alloc_allocation(void *user_data, size_t size, size_t alignment,
    ...
    DTRACE("Allocated block %p. Size=%llu, Alignment=%llu", result, size,
           alignment);
#endif
    return result;
}
...
void vulkan_alloc_free(void *user_data, void *memory) {
    ...
    if (result) {
        ...
        DERROR("vulkan_alloc_free failed to get alignment lookup for block %p.",
               memory);
    }
}
...
b8 vulkan_renderer_backend_initialize(renderer_plugin *plugin,
    ...
    if (context->validation_enabled) {
        ...
        for (u32 i = 0; i < required_validation_layer_count; ++i) {
            ...
            for (u32 j = 0; j < available_layer_count; ++j) {
                if (strings_equal(required_validation_layer_names[i],
                    ...
                    found = true;
                    DINFO("Found validation layer: %s...",
            ...
            if (!found) {
                DFATAL("Required validation layer is missing: %s",
                       required_validation_layer_names[i]);
                return false;
            }
        }
        ...
        DINFO("Vulkan validation layers are not enabled.");
    }
    ...
    *out_window_render_target_count = context->swapchain.image_count;
    ...
    if (!renderer_renderbuffer_create("staging", RENDERBUFFER_TYPE_STAGING, staging_buffer_size, RENDERBUFFER_TRACK_TYPE_LINEAR, &context->staging)) {
        DERROR("Failed to create staging buffer.");
        return false;
    }
...
void vulkan_renderer_backend_on_resized(renderer_plugin *plugin, u16 width,
    ...
    DINFO("Vulkan renderer plugin->resized: w/h/gen: %i/%i/%llu", width, height,
          context->framebuffer_size_generation);
}
...
b8 vulkan_renderer_frame_prepare(renderer_plugin *plugin, struct frame_data *p_frame_data) {
    ...
    if (!renderer_renderbuffer_clear(&context->staging, false)) {
        DERROR("Failed to clear staging buffer.");
        return false;
    }
...
b8 vulkan_renderer_end(renderer_plugin *plugin, struct frame_data *p_frame_data) {
    ...
    if (result != VK_SUCCESS) {
        DERROR("vkQueueSubmit failed with result: %s",
...
void vulkan_renderer_winding_set(struct renderer_plugin *plugin, renderer_winding winding) {
    ...
    if (context->device.support_flags & VULKAN_DEVICE_SUPPORT_FLAG_NATIVE_DYNAMIC_STATE_BIT) {
        ...
        DFATAL("renderer_winding_set cannot be used on a device without dynamic state support.");
    }
}
...
static VkStencilOp vulkan_renderer_get_stencil_op(renderer_stencil_op op) {
    switch (op) {
        ...
        default:
            DWARN("Unsupported stencil op, defaulting to keep.");
            return VK_STENCIL_OP_KEEP;
    }
}
...
static VkCompareOp vulkan_renderer_get_compare_op(renderer_compare_op op) {
    switch (op) {
        ...
        default:
            DWARN("Unsupported compare op, using always.");
            return VK_COMPARE_OP_ALWAYS;
    }
}
...
void vulkan_renderer_set_stencil_test_enabled(struct renderer_plugin *plugin, b8 enabled) {
    ...
    if (context->device.support_flags & VULKAN_DEVICE_SUPPORT_FLAG_NATIVE_DYNAMIC_STATE_BIT) {
        ...
        DFATAL("renderer_set_stencil_test_enabled cannot be used on a device without dynamic state support.");
    }
}
...
void vulkan_renderer_set_depth_test_enabled(struct renderer_plugin *plugin, b8 enabled) {
    ...
    if (context->device.support_flags & VULKAN_DEVICE_SUPPORT_FLAG_NATIVE_DYNAMIC_STATE_BIT) {
        ...
        DFATAL("renderer_set_depth_test_enabled cannot be used on a device without dynamic state support.");
    }
}
...
void vulkan_renderer_set_stencil_op(struct renderer_plugin *plugin, renderer_stencil_op fail_op, renderer_stencil_op pass_op, renderer_stencil_op depth_fail_op, renderer_compare_op compare_op) {
    ...
    if (context->device.support_flags & VULKAN_DEVICE_SUPPORT_FLAG_NATIVE_DYNAMIC_STATE_BIT) {
        ...
        DFATAL("renderer_set_stencil_op cannot be used on a device without dynamic state support.");
    }
}
...
b8 vulkan_renderer_renderpass_begin(renderer_plugin *plugin, renderpass *pass, render_target *target) {
    ...
    // DTRACE("Renderpass '%s' is using framebuffer at 0x%x", pass->name, target->internal_framebuffer);
...
static b8 recreate_swapchain(vulkan_context *context) {
    ...
    event_execute(EVENT_CODE_DEFAULT_RENDERTARGET_REFRESH_REQUIRED, 0,
               event_context);
    ...
    for (u8 i = 0; i < config->stage_count; ++i) {
        switch (config->stage_configs[i].stage) {
            ...
            case SHADER_STAGE_COMPUTE:
                ...
                break;
            default:
                DERROR("Unsupported stage type: %d", config->stage_configs[i].name);
                break;
        }
    }
...
static b8 shader_create_modules_and_pipelines(renderer_plugin *plugin, shader *s) {
    ...
    for (u32 i = 0; i < internal_shader->stage_count; ++i) {
        if (!create_shader_module(context, s, &s->stage_configs[i], &new_stages[i])) {
            DERROR("Unable to create %s shader module for '%s'. Shader will be destroyed.", s->stage_configs[i].filename, s->name);
            has_error = true;
    ...
    for (u32 i = 0; i < pipeline_count; ++i) {
        ...
        if (!pipeline_result) {
            DERROR("Failed to load graphics pipeline for shader: '%s'.", s->name);
            has_error = true;
            break;
        }
    }
...
b8 vulkan_renderer_shader_initialize(renderer_plugin *plugin, shader *s) {
    ...
    if (!context->device.features.fillModeNonSolid) {
        DINFO("Renderer backend does not support fillModeNonSolid. Wireframe mode is not possible, but was requested for the shader '%s'.", s->name);
    ...
    for (u32 i = 0; i < internal_shader->descriptor_set_count; ++i) {
        ...
        if (!vulkan_result_is_success(result)) {
            DERROR("vulkan_shader_initialize failed descriptor set layout: '%s'", vulkan_result_string(result, true));
            return false;
        }
    }
    ...
    if (!shader_create_modules_and_pipelines(plugin, s)) {
        DERROR("Failed initial load on shader '%s'. See logs for details.", s->name);
        return false;
    }
                ...
                if (internal_shader->pipelines[i]->supported_topology_types & j) {
                    switch (j) {
                        ...
                        case PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE_FAN:
                            ...
                            break;
                        default:
                            DWARN("primitive topology '%u' not supported. Skipping.", j);
                            break;
                    }
    ...
    if (!pipeline_found) {
        ...
        DERROR("No available topology classes are available, so a pipeline cannot be bound. Check shader configuration.");
        return false;
    }
...
b8 vulkan_renderer_shader_bind_instance(renderer_plugin *plugin, shader *s, u32 instance_id) {
    ...
    if (instance_id == INVALID_ID) {
        DERROR("Cannot bind instance INVALID_ID.");
        return false;
    }
    vulkan_shader *internal = s->internal_data;
                ...
                for (u32 d = 0; d < binding_descriptor_count; ++d) {
                    ...
                    if (t->generation == INVALID_ID) {
                        ...
                        if (t->generation != map->generation) {
                            ...
                            DTRACE("A sampler refresh is%s required. Tex/map mips: %u/%u", refresh_required ? "" : " not", t->mip_levels, map->mip_levels);
                            if (refresh_required && !vulkan_renderer_texture_map_resources_refresh(plugin, map)) {
                                DWARN("Failed to refresh texture map resources. This means the sampler settings could be out of date.");
                            } else {
                    ...
                    if (t->generation == INVALID_ID) {
                        ...
                        if (!texture_system_is_default_texture(t)) {
                            ...
                            if (t->type == TEXTURE_TYPE_2D) {
                                ...
                                DERROR("Vulkan renderer failed to determine texture type while applying instance. Falling back to 2D.");
                                t = texture_system_get_default_texture();
                            }
                        }
                        // If using the default texture, invalidate the map's generation so it's updated next run.
...
b8 vulkan_renderer_shader_instance_resources_acquire(renderer_plugin *plugin, struct shader *s, const shader_instance_resource_config *config, u32 *out_instance_id) {
    ...
    *out_instance_id = INVALID_ID;
    ...
    if (*out_instance_id == INVALID_ID) {
        DERROR("vulkan_shader_acquire_instance_resources failed to acquire new id for shader '%s', max instances=%u", s->name, internal->max_instances);
        return false;
    }
...
static b8 sampler_state_try_set(vulkan_uniform_sampler_state *sampler_uniforms, u32 sampler_count, u16 uniform_location, u32 array_index, texture_map *map) {
        ...
        if (su->uniform->location == uniform_location) {
            if (su->uniform->array_length > 1) {
                if (array_index >= su->uniform->array_length) {
                    DERROR("vulkan_renderer_uniform_set error: array_index (%u) is out of range (0-%u)", array_index, su->uniform->array_length);
                    return false;
                }
    ...
    DERROR("sampler_state_try_set: Unable to find uniform location %u. Sampler uniform not set.", uniform_location);
    return false;
}
...
static b8 create_shader_module(vulkan_context *context, shader *s, shader_stage_config *config, vulkan_shader_stage *out_stage) {
    ...
    switch (config->stage) {
        ...
        case SHADER_STAGE_GEOMETRY:
            ...
            break;
        default:
            DERROR("Unsupported shader kind. Unable to create module.");
            return false;
    }

    DDEBUG("Compiling stage '%s' for shader '%s'...", shader_type_str, s->name);
    ...
    if (!compilation_result) {
        DERROR("An unknown error occurred while trying to compile the shader. Unable to process futher.");
        return false;
    }
    ...
    if (status != shaderc_compilation_status_success) {
        ...
        DERROR("Error compiling shader with %llu errors.", error_count);
        DERROR("Error(s):\n%s", error_message);
        shaderc_result_release(compilation_result);
        return false;
    }

    DDEBUG("Shader compiled successfully.");
    ...
    for (u32 i = 0; i < config->target.attachment_count; ++i) {
        ...
        if (attachment_config->type == RENDER_TARGET_ATTACHMENT_TYPE_COLOUR) {
            ...
            if (attachment_config->store_operation == RENDER_TARGET_ATTACHMENT_STORE_OPERATION_DONT_CARE) {
                ...
                DFATAL("Invalid store operation (0x%x) set for colour attachment. Check configuration.", attachment_config->store_operation);
                return false;
            }
...
b8 vulkan_renderer_render_target_create(renderer_plugin *plugin,
    ...
    DTRACE("Created framebuffer ' %s' at 0x%x.", formatted_name, out_target->internal_framebuffer);
    return true;
}
...
b8 vulkan_buffer_create_internal(renderer_plugin *plugin, renderbuffer *buffer) {
    ...
    if (!vulkan_result_is_success(result)) {
        DERROR("Failed to allocate memory for buffer with error: %s", vulkan_result_string(result, true));
        return false;
    }
