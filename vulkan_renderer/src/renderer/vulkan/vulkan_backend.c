...
void vulkan_alloc_free(void* user_data, void* memory) {
    ...
    if (result) {
        ...
        DERROR("vulkan_alloc_free failed to get alignment lookup for block %p.", memory);
    }
}
...
void* vulkan_alloc_reallocation(void* user_data, void* original, size_t size, size_t alignment, VkSystemAllocationScope allocation_scope) {
    ...
    if (!is_aligned) {
        DERROR("vulkan_alloc_reallocation of unaligned block %p", original);
        return 0;
    }

    if (alloc_alignment != alignment) {
        DERROR("Attempted realloc using a different alignment of %llu than the original of %hu.", alignment, alloc_alignment);
        return 0;
    }
...
#ifdef KVULKAN_ALLOCATOR_TRACE
        DERROR("Failed to realloc %p.", original);
#endif
    }
...
b8 vulkan_renderer_backend_initialize(renderer_plugin* plugin, const renderer_backend_config* config, u8* out_window_render_target_count) {
    ...
    if (!create_vulkan_allocator(context, context->allocator)) {
        ...
        DFATAL("Failed to create custom Vulkan allocator. Continuing using the driver's default allocator.");
    ...
    for (u32 i = 0; i < required_extension_count; ++i) {
        ...
        for (u32 j = 0; j < available_extension_count; ++j) {
            if (strings_equal(required_extensions[i], available_extensions[j].extensionName)) {
                found = true;
                DINFO("Required exension found: %s...", required_extensions[i]);
                break;
            }
        }

        if (!found) {
            DFATAL("Required extension is missing: %s", required_extensions[i]);
            return false;
        }
    }
...
// and make sure they exist. Validation layers should only be enabled on non-release builds.
    ...
    for (u32 i = 0; i < required_validation_layer_count; ++i) {
        ...
        for (u32 j = 0; j < available_layer_count; ++j) {
            if (strings_equal(required_validation_layer_names[i], available_layers[j].layerName)) {
                found = true;
                DINFO("Found validation layer: %s...", required_validation_layer_names[i]);
                break;
            }
        }
    ...
    if (!vulkan_result_is_success(instance_result)) {
        ...
        DFATAL("Vulkan instance creation failed with result: '%s'", result_string);
        return false;
    }
...
void vulkan_renderer_backend_on_resized(renderer_plugin* plugin, u16 width, u16 height) {
    ...
    DINFO("Vulkan renderer plugin->resized: w/h/gen: %i/%i/%llu", width, height, context->framebuffer_size_generation);
}
...
b8 vulkan_renderer_backend_frame_begin(renderer_plugin* plugin, const struct frame_data* p_frame_data) {
    ...
    if (context->recreating_swapchain) {
        ...
        if (!vulkan_result_is_success(result)) {
            DERROR("vulkan_renderer_backend_frame_begin vkDeviceWaitIdle (1) failed: '%s'", vulkan_result_string(result, true));
            return false;
        }
    ...
    if (context->framebuffer_size_generation != context->framebuffer_size_last_generation || context->render_flag_changed) {
        VkResult result = vkDeviceWaitIdle(device->logical_device);
        if (!vulkan_result_is_success(result)) {
            DERROR("vulkan_renderer_backend_frame_begin vkDeviceWaitIdle (2) failed: '%s'", vulkan_result_string(result, true));
            return false;
        }
    ...
    if (!vulkan_swapchain_acquire_next_image_index(
            ...
            0,
            &context->image_index)) {
        DERROR("Failed to acquire next image index, booting.");
        return false;
    }
...
static b8 recreate_swapchain(vulkan_context* context) {
    ...
    event_execute(EVENT_CODE_DEFAULT_RENDERTARGET_REFRESH_REQUIRED, 0, event_context);
...
void vulkan_renderer_texture_write_data(renderer_plugin* plugin, texture* t, u32 offset, u32 size, const u8* pixels) {
    ...
    if (!renderer_renderbuffer_create(RENDERBUFFER_TYPE_STAGING, size, false, &staging)) {
        DERROR("Failed to create staging buffer for texture write.");
        return;
    }
...
void vulkan_renderer_texture_read_data(renderer_plugin* plugin, texture* t, u32 offset, u32 size, void** out_memory) {
    ...
    if (!renderer_renderbuffer_create(RENDERBUFFER_TYPE_READ, size, false, &staging)) {
        DERROR("Failed to create staging buffer for texture read.");
        return;
    }
    renderer_renderbuffer_bind(&staging, 0);
    ...
    if (!vulkan_buffer_read(plugin, &staging, offset, size, out_memory)) {
        DERROR("vulkan_buffer_read failed.");
    }
...
void vulkan_renderer_texture_read_pixel(renderer_plugin* plugin, texture* t, u32 x, u32 y, u8** out_rgba) {
    ...
    if (!renderer_renderbuffer_create(RENDERBUFFER_TYPE_READ, sizeof(u8) * 4, false, &staging)) {
        DERROR("Failed to create staging buffer for texture pixel read.");
        return;
    }
    renderer_renderbuffer_bind(&staging, 0);
...
b8 vulkan_renderer_geometry_create(renderer_plugin* plugin, geometry* geometry, u32 vertex_size, u32 vertex_count, const void* vertices, u32 index_size, u32 index_count, const void* indices) {
    ...
    if (!vertex_count || !vertices) {
        DERROR("vulkan_renderer_geometry_create requires vertex data, and none was supplied. vertex_count=%d, vertices=%p", vertex_count, vertices);
        return false;
    }
    ...
    if (!internal_data) {
        DFATAL("vulkan_renderer_geometry_create failed to find a free index for a new geometry upload. Adjust config to allow for more.");
        return false;
    }
    ...
    if (!renderer_renderbuffer_allocate(&context->object_vertex_buffer, total_size, &internal_data->vertex_buffer_offset)) {
        DERROR("vulkan_renderer_geometry_create failed to allocate from the vertex buffer!");
        return false;
    }

    // Load the data.
    if (!renderer_renderbuffer_load_range(&context->object_vertex_buffer, internal_data->vertex_buffer_offset, total_size, vertices)) {
        DERROR("vulkan_renderer_geometry_create failed to upload to the vertex buffer!");
        return false;
    }
    ...
    if (index_count && indices) {
        ...
        if (!renderer_renderbuffer_allocate(&context->object_index_buffer, total_size, &internal_data->index_buffer_offset)) {
            DERROR("vulkan_renderer_geometry_create failed to allocate from the index buffer!");
            return false;
        }

        if (!renderer_renderbuffer_load_range(&context->object_index_buffer, internal_data->index_buffer_offset, total_size, indices)) {
            DERROR("vulkan_renderer_geometry_create failed to upload to the index buffer!");
            return false;
        }
    }
    ...
    if (is_reupload) {
        ...
        if (!renderer_renderbuffer_free(&context->object_vertex_buffer, old_range.vertex_element_size * old_range.vertex_count, old_range.vertex_buffer_offset)) {
            DERROR("vulkan_renderer_geometry_create free operation failed during reupload of vertex data.");
            return false;
        }
        ...
        if (old_range.index_element_size > 0) {
            if (!renderer_renderbuffer_free(&context->object_index_buffer, old_range.index_element_size * old_range.index_count, old_range.index_buffer_offset)) {
                DERROR("vulkan_renderer_geometry_create free operation failed during reupload of index data.");
                return false;
            }
        }
    }
...
void vulkan_renderer_geometry_destroy(renderer_plugin* plugin, geometry* geometry) {
    ...
    if (geometry && geometry->internal_id != INVALID_ID) {
        ...
        if (!renderer_renderbuffer_free(&context->object_vertex_buffer, internal_data->vertex_element_size * internal_data->vertex_count, internal_data->vertex_buffer_offset)) {
            DERROR("vulkan_renderer_geometry_destroy failed to free vertex buffer range.");
        }
        ...
        if (internal_data->index_element_size > 0) {
            if (!renderer_renderbuffer_free(&context->object_index_buffer, internal_data->index_element_size * internal_data->index_count, internal_data->index_buffer_offset)) {
                DERROR("vulkan_renderer_geometry_destroy failed to free index buffer range.");
            }
        }
...
void vulkan_renderer_geometry_draw(renderer_plugin* plugin, geometry_render_data* data) {
    ...
    if (!vulkan_buffer_draw(plugin, &context->object_vertex_buffer, buffer_data->vertex_buffer_offset, buffer_data->vertex_count, includes_index_data)) {
        DERROR("vulkan_renderer_geometry_draw failed to draw vertex buffer;");
        return;
    }
    ...
    if (includes_index_data) {
        if (!vulkan_buffer_draw(plugin, &context->object_index_buffer, buffer_data->index_buffer_offset, buffer_data->index_count, !includes_index_data)) {
            DERROR("vulkan_renderer_geometry_draw failed to draw index buffer;");
            return;
        }
    }
}
...
b8 vulkan_renderer_shader_create(renderer_plugin* plugin, shader* s, const shader_config* config, renderpass* pass, u8 stage_count, const char** stage_filenames, shader_stage* stages) {
    ...
    for (u8 i = 0; i < stage_count; ++i) {
        switch (stages[i]) {
            ...
            case SHADER_STAGE_GEOMETRY:
                DWARN("vulkan_renderer_shader_create: VK_SHADER_STAGE_GEOMETRY_BIT is set but not yet supported.");
                vk_stages[i] = VK_SHADER_STAGE_GEOMETRY_BIT;
                break;
            case SHADER_STAGE_COMPUTE:
                DWARN("vulkan_renderer_shader_create: SHADER_STAGE_COMPUTE is set but not yet supported.");
                vk_stages[i] = VK_SHADER_STAGE_COMPUTE_BIT;
                break;
            default:
                DERROR("Unsupported stage type: %d", stages[i]);
                break;
        }
    }
    ...
    for (u32 i = 0; i < stage_count; i++) {
        ...
        if (internal_shader->config.stage_count + 1 > VULKAN_SHADER_MAX_STAGES) {
            DERROR("Shaders may have a maximum of %d stages", VULKAN_SHADER_MAX_STAGES);
            return false;
        }
        ...
        switch (stages[i]) {
            ...
            default:
                // Go to the next type.
                DERROR("vulkan_shader_create: Unsupported shader stage flagged: %d. Stage ignored.", stages[i]);
                continue;
        }
...
void vulkan_renderer_shader_destroy(renderer_plugin* plugin, shader* s) {
    ...
    if (s && s->internal_data) {
        ...
        if (!shader) {
            DERROR("vulkan_renderer_shader_destroy requires a valid pointer to a shader.");
            return;
        }
...
b8 vulkan_renderer_shader_initialize(renderer_plugin* plugin, shader* s) {
    ...
    for (u32 i = 0; i < internal_shader->config.stage_count; ++i) {
        if (!create_shader_module(context, internal_shader, internal_shader->config.stages[i], &internal_shader->stages[i])) {
            DERROR("Unable to create %s shader module for '%s'. Shader will be destroyed.", internal_shader->config.stages[i].file_name, s->name);
            return false;
        }
    }
    ...
    if (!vulkan_result_is_success(result)) {
        DERROR("vulkan_shader_initialize failed creating descriptor pool: '%s'", vulkan_result_string(result, true));
        return false;
    }
...
b8 vulkan_renderer_shader_apply_instance(renderer_plugin* plugin, shader* s, b8 needs_update) {
    ...
    if (internal->instance_uniform_count < 1 && internal->instance_uniform_sampler_count < 1) {
        DERROR("This shader does not use instances.");
        return false;
    }
    u32 image_index = context->image_index;
    VkCommandBuffer command_buffer = context->graphics_command_buffers[image_index].handle;
                ...
                if (t->generation == INVALID_ID) {
                    switch (map->use) {
                        ...
                        case TEXTURE_USE_MAP_NORMAL:
                            ...
                            break;
                        default:
                            DWARN("Undefined texture use %d", map->use);
...
static VkSamplerAddressMode convert_repeat_type(const char* axis, texture_repeat repeat) {
    switch (repeat) {
        ...
        default:
            DWARN("convert_repeat_type(axis='%s') Type '%x' not supported, defaulting to repeat.", axis, repeat);
            return VK_SAMPLER_ADDRESS_MODE_REPEAT;
    }
}
...
static VkFilter convert_filter_type(const char* op, texture_filter filter) {
    switch (filter) {
        ...
        default:
            DWARN("convert_filter_type(op='%s'): Unsupported filter type '%x', defaulting to linear.", op, filter);
            return VK_FILTER_LINEAR;
    }
}
...
b8 vulkan_renderer_shader_instance_resources_acquire(renderer_plugin* plugin, shader* s, texture_map** maps, u32* out_instance_id) {
    ...
    *out_instance_id = INVALID_ID;
    ...
    if (*out_instance_id == INVALID_ID) {
        DERROR("vulkan_shader_acquire_instance_resources failed to acquire new id");
        return false;
    }
    ...
    if (size > 0) {
        if (!renderer_renderbuffer_allocate(&internal->uniform_buffer, size, &instance_state->offset)) {
            DERROR("vulkan_material_shader_acquire_resources failed to acquire ubo space");
            return false;
        }
    }
    ...
    if (result != VK_SUCCESS) {
        DERROR("Error allocating instance descriptor sets in shader: '%s'.", vulkan_result_string(result, true));
        return false;
    }
...
b8 vulkan_renderer_shader_instance_resources_release(renderer_plugin* plugin, shader* s, u32 instance_id) {
    ...
    if (s->ubo_stride != 0) {
        if (!renderer_renderbuffer_free(&internal->uniform_buffer, s->ubo_stride, instance_state->offset)) {
            DERROR("vulkan_renderer_shader_instance_resources_release failed to free range from renderbuffer.");
        }
    }
...
static b8 create_shader_module(vulkan_context* context, vulkan_shader* shader, vulkan_shader_stage_config config, vulkan_shader_stage* shader_stage) {
    ...
    if (!resource_system_load(config.file_name, RESOURCE_TYPE_BINARY, 0, &binary_resource)) {
        DERROR("Unable to read shader module: %s.", config.file_name);
        return false;
    }
        ...
        if (attachment_config->type == RENDER_TARGET_ATTACHMENT_TYPE_COLOUR) {
            ...
            if (attachment_config->load_operation == RENDER_TARGET_ATTACHMENT_LOAD_OPERATION_DONT_CARE) {
                ...
                if (attachment_config->load_operation == RENDER_TARGET_ATTACHMENT_LOAD_OPERATION_LOAD) {
                    if (do_clear_colour) {
                        DWARN("Colour attachment load operation set to load, but is also set to clear. This combination is invalid, and will err toward clearing. Verify attachment configuration.");
                        attachment_desc.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
                    } else {
                        attachment_desc.loadOp = VK_ATTACHMENT_LOAD_OP_LOAD;
                    }
                } else {
                    DFATAL("Invalid and unsupported combination of load operation (0x%x) and clear flags (0x%x) for colour attachment.", attachment_desc.loadOp, out_renderpass->clear_flags);
                    return false;
                }
            }
            ...
            if (attachment_config->store_operation == RENDER_TARGET_ATTACHMENT_STORE_OPERATION_DONT_CARE) {
                ...
                DFATAL("Invalid store operation (0x%x) set for depth attachment. Check configuration.", attachment_config->store_operation);
                return false;
            }
            ...
            if (attachment_config->load_operation == RENDER_TARGET_ATTACHMENT_LOAD_OPERATION_DONT_CARE) {
                ...
                if (attachment_config->load_operation == RENDER_TARGET_ATTACHMENT_LOAD_OPERATION_LOAD) {
                    if (do_clear_depth) {
                        DWARN("Depth attachment load operation set to load, but is also set to clear. This combination is invalid, and will err toward clearing. Verify attachment configuration.");
                        attachment_desc.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
                    } else {
                        attachment_desc.loadOp = VK_ATTACHMENT_LOAD_OP_LOAD;
                    }
                } else {
                    DFATAL("Invalid and unsupported combination of load operation (0x%x) and clear flags (0x%x) for depth attachment.", attachment_desc.loadOp, out_renderpass->clear_flags);
                    return false;
                }
            }
...
texture* vulkan_renderer_window_attachment_get(renderer_plugin* plugin, u8 index) {
    ...
    if (index >= context->swapchain.image_count) {
        DFATAL("Attempting to get colour attachment index out of range: %d. Attachment count: %d", index, context->swapchain.image_count);
        return 0;
    }
...
texture* vulkan_renderer_depth_attachment_get(renderer_plugin* plugin, u8 index) {
    vulkan_context* context = (vulkan_context*)plugin->internal_context;
    if (index >= context->swapchain.image_count) {
        DFATAL("Attempting to get depth attachment index out of range: %d. Attachment count: %d", index, context->swapchain.image_count);
        return 0;
    }
...
b8 vulkan_buffer_create_internal(renderer_plugin* plugin, renderbuffer* buffer) {
    vulkan_context* context = (vulkan_context*)plugin->internal_context;
    if (!buffer) {
        DERROR("vulkan_buffer_create_internal requires a valid pointer to a buffer.");
        return false;
    }
    ...
    switch (buffer->type) {
        ...
        case RENDERBUFFER_TYPE_STORAGE:
            DERROR("Storage buffer not yet supported.");
            return false;
        default:
            DERROR("Unsupported buffer type: %i", buffer->type);
            return false;
    }
...
b8 vulkan_buffer_bind(renderer_plugin* plugin, renderbuffer* buffer, u64 offset) {
    vulkan_context* context = (vulkan_context*)plugin->internal_context;
    if (!buffer || !buffer->internal_data) {
        DERROR("vulkan_buffer_bind requires valid pointer to a buffer.");
        return false;
    }
    vulkan_buffer* internal_buffer = (vulkan_buffer*)buffer->internal_data;
...
b8 vulkan_buffer_unbind(renderer_plugin* plugin, renderbuffer* buffer) {
    if (!buffer || !buffer->internal_data) {
        DERROR("vulkan_buffer_unbind requires valid pointer to a buffer.");
        return false;
    }
...
void* vulkan_buffer_map_memory(renderer_plugin* plugin, renderbuffer* buffer, u64 offset, u64 size) {
    vulkan_context* context = (vulkan_context*)plugin->internal_context;
    if (!buffer || !buffer->internal_data) {
        DERROR("vulkan_buffer_map_memory requires a valid pointer to a buffer.");
        return 0;
    }
    vulkan_buffer* internal_buffer = (vulkan_buffer*)buffer->internal_data;
    void* data;
...
void vulkan_buffer_unmap_memory(renderer_plugin* plugin, renderbuffer* buffer, u64 offset, u64 size) {
    vulkan_context* context = (vulkan_context*)plugin->internal_context;
    if (!buffer || !buffer->internal_data) {
        DERROR("vulkan_buffer_unmap_memory requires a valid pointer to a buffer.");
        return;
    }
    vulkan_buffer* internal_buffer = (vulkan_buffer*)buffer->internal_data;
...
b8 vulkan_buffer_flush(renderer_plugin* plugin, renderbuffer* buffer, u64 offset, u64 size) {
    vulkan_context* context = (vulkan_context*)plugin->internal_context;
    if (!buffer || !buffer->internal_data) {
        DERROR("vulkan_buffer_flush requires a valid pointer to a buffer.");
        return false;
    }
...
b8 vulkan_buffer_read(renderer_plugin* plugin, renderbuffer* buffer, u64 offset, u64 size, void** out_memory) {
    ...
    if (!buffer || !buffer->internal_data || !out_memory) {
        DERROR("vulkan_buffer_read requires a valid pointer to a buffer and out_memory, and the size must be nonzero.");
        return false;
    }
    ...
    if (vulkan_buffer_is_device_local(plugin, internal_buffer) && !vulkan_buffer_is_host_visible(plugin, internal_buffer)) {
        ...
        if (!renderer_renderbuffer_create(RENDERBUFFER_TYPE_READ, size, false, &read)) {
            DERROR("vulkan_buffer_read() - Failed to create read buffer.");
            return false;
        }
...
b8 vulkan_buffer_load_range(renderer_plugin* plugin, renderbuffer* buffer, u64 offset, u64 size, const void* data) {
    ...
    if (!buffer || !buffer->internal_data || !size || !data) {
        DERROR("vulkan_buffer_load_range requires a valid pointer to a buffer, a nonzero size and a valid pointer to data.");
        return false;
    }
    ...
    if (vulkan_buffer_is_device_local(plugin, internal_buffer) && !vulkan_buffer_is_host_visible(plugin, internal_buffer)) {
        ...
        renderbuffer staging;
        if (!renderer_renderbuffer_create(RENDERBUFFER_TYPE_STAGING, size, false, &staging)) {
            DERROR("vulkan_buffer_load_range() - Failed to create staging buffer.");
            return false;
        }
        renderer_renderbuffer_bind(&staging, 0);
...
b8 vulkan_buffer_copy_range(renderer_plugin* plugin, renderbuffer* source, u64 source_offset, renderbuffer* dest, u64 dest_offset, u64 size) {
    ...
    if (!source || !source->internal_data || !dest || !dest->internal_data || !size) {
        DERROR("vulkan_buffer_copy_range requires a valid pointers to source and destination buffers as well as a nonzero size.");
        return false;
    }
...
b8 vulkan_buffer_draw(renderer_plugin* plugin, renderbuffer* buffer, u64 offset, u32 element_count, b8 bind_only) {
    ...
    if (buffer->type == RENDERBUFFER_TYPE_VERTEX) {
        ...
        DERROR("Cannot draw buffer of type: %i", buffer->type);
        return false;
    }
}