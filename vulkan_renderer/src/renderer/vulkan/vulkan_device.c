...
b8 vulkan_device_create(vulkan_context* context) {
    ...
    if (
        ...
        DINFO("Vulkan device doesn't support native dynamic topology, but does via extension. Using extension.");
        ...
        if (context->device.support_flags & VULKAN_DEVICE_SUPPORT_FLAG_NATIVE_DYNAMIC_TOPOLOGY_BIT) {
            DINFO("Vulkan device supports native dynamic topology.");
        } else {
            DINFO("Vulkan device does not support native or extension dynamic topology.");
        }
    }
    ...
    if (
        ...
        DINFO("Vulkan device doesn't support native dynamic front-face, but does via extension. Using extension.");
        ...
        if (context->device.support_flags & VULKAN_DEVICE_SUPPORT_FLAG_NATIVE_DYNAMIC_FRONT_FACE_BIT) {
            DINFO("Vulkan device supports native dynamic front-face.");
        } else {
            DINFO("Vulkan device does not support native or extension dynamic front-face.");
        }
    }
    // Get queues.
    vkGetDeviceQueue(
...
static b8 select_physical_device(vulkan_context* context) {
    ...
    for (u32 i = 0; i < physical_device_count; ++i) {
        ...
        DINFO("Evaluating device: '%s', index %u.", properties.deviceName, i);
        ...
        if (result) {
            ...
            DINFO("GPU Driver version: %s", driverProperties.driverInfo);
        ...
        for (u32 i = 0; i < queue_family_count; ++i) {
            ...
            if (supports_present) {
                ...
                if (out_queue_info->present_family_index != out_queue_info->graphics_family_index) {
                    DWARN("Warning: Different queue index used for present vs graphics: %u.", i);
                }
                break;
            }
        }
