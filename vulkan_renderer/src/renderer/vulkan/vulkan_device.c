...
b8 vulkan_device_create(vulkan_context* context) {
    ...
    if (
        ...
        DINFO("Vulkan device doesn't support native dynamic state, but does via extension. Using extension.");
        ...
        if (context->device.support_flags & VULKAN_DEVICE_SUPPORT_FLAG_NATIVE_DYNAMIC_STATE_BIT) {
            DINFO("Vulkan device supports native dynamic state.");
        } else {
            DWARN("Vulkan device does not support native or extension dynamic state. This may cause issues with the renderer.");
        }
    }
