...
void vulkan_image_create(
    ...
    if (mip_levels < 1) {
        DWARN("Mip levels must be >= 1. Defaulting to 1.");
        mip_levels = 1;
    }
    // Copy params
...
// A lookup table of vulkan image view types indexed texture types.
...
b8 vulkan_image_mipmaps_generate(vulkan_context* context, vulkan_image* image, vulkan_command_buffer* command_buffer) {
    if (image->mip_levels <= 1) {
        DWARN("Attempted to generate mips for an image that isn't configured for them.");
        return false;
    }
    ...
    if (!(format_properties.optimalTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT)) {
        DWARN("Texture image format does not support linear blitting! Mipmaps cannot be created.");
        return false;
    }
